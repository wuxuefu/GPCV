...
#define __MODULE__          "Cvr_Recorde_Main"
...
static pthread_t cloudDogThread = 0;
UINT8 Playmode_flag;
	...
	printf("enter menu_select_count= %d,item_select_count = %d flag %d\n",dv_set.menu_select_count,dv_set.item_select_count, dv_set.menu_select_flag);
		...
		else if((dv_set.menu_select_count == DV_LDW)&&(Playmode_flag == CMD_VIDEO_MODE)&&(dv_set.dv_UI_flag == DV_MENU_MODE))
		{
			...
			else if(dv_set.menu_select_flag == 2)
			{
				...
				}else if(dv_set.item_select_count == 6)//sound
				{
					ap_state_config_LDW_SFCW_set(dv_set.item_select_count1);					
				}else if(dv_set.item_select_count == 7)//sound
				{
...
)
{
    ...
    if(cloudDogThread == 0)
    {
        ...
        pthread_create(&cloudDogThread,NULL,CloudDog_Thread,&cloudDogThread);
    }
	...
	if(cloudDogThread) {
	    pthread_join(cloudDogThread,NULL);
	    cloudDogThread = 0;
	}
	dispDestroy(hDisp);
