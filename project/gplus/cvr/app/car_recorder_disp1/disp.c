...
)
{
	...
	{
		disp_pipemsg_send1(DISP_BUF_READ_UI, 4, &ret);
		return;
	}
	if (pDisp->layer == DISP_LAYER_PRIMARY) {
...
void drawLDWLine(unsigned short* drawImg, int startx, int starty, int endx, int endy,int width, int colorR, int colorG, int colorB)
{//min=3,
	int i,j;
	int wid_y_up=0,wid_y_dn=0,wid_x_left=0,wid_x_right=0;
	unsigned short colorRGB565;
	...
	{
		starty=starty*3/4+30;		
		endy=endy*3/4+30;
	}
	if(width==3)
	{
		drawLine(drawImg, startx, starty, endx, endy, colorR, colorG, colorB);
		drawLine(drawImg, startx-1, starty, endx-1, endy, colorR, colorG, colorB);
		drawLine(drawImg, startx+1, starty, endx+1, endy, colorR, colorG, colorB);
		drawLine(drawImg, startx, starty-1, endx, endy-1, colorR, colorG, colorB);
		drawLine(drawImg, startx, starty+1, endx, endy+1, colorR, colorG, colorB);		
	}else if(width==5)
	{
		drawLine(drawImg, startx, starty, endx, endy, colorR, colorG, colorB);
		drawLine(drawImg, startx-1, starty, endx-1, endy, colorR, colorG, colorB);
		drawLine(drawImg, startx-2, starty, endx-2, endy, colorR, colorG, colorB);
		drawLine(drawImg, startx+1, starty, endx+1, endy, colorR, colorG, colorB);
		drawLine(drawImg, startx+2, starty, endx+2, endy, colorR, colorG, colorB);
		drawLine(drawImg, startx, starty-1, endx, endy-1, colorR, colorG, colorB);
		drawLine(drawImg, startx, starty-2, endx, endy-2, colorR, colorG, colorB);
		drawLine(drawImg, startx, starty+1, endx, endy+1, colorR, colorG, colorB);		
		drawLine(drawImg, startx, starty+2, endx, endy+2, colorR, colorG, colorB);	

		drawLine(drawImg, startx+1, starty+1, endx+1, endy+1, colorR, colorG, colorB);		
		drawLine(drawImg, startx+1, starty-1, endx+1, endy-1, colorR, colorG, colorB);	
	}
	...
	colorRGB565 = colorRGB565<<5;
	colorRGB565 += colorB;
//start && end point  changed to rect ,
if(width<3)
{
	width=3;
}
	if(width%2)//width is odd line
	{
		wid_y_up=wid_y_dn=wid_x_left=wid_x_right=width/2;	
	}else
	{//width is even line
		wid_y_up=wid_x_right=width/2;
		wid_y_dn=wid_x_left=wid_y_up-1;
	}
	
	
	for(i=1;i<=wid_y_up;i++)
	{
		drawLine(drawImg, startx, starty-i-1, endx, endy-i+1, colorR, colorG, colorB);
	}	
	for(i=1;i<=wid_y_dn;i++)
	{
		drawLine(drawImg, startx, starty+i, endx, endy+i, colorR, colorG, colorB);
	}	
	

	drawLine(drawImg, startx-wid_x_left, starty+1, endx-wid_x_left, endy-1, colorR, colorG, colorB);
	for(i=1;i<=wid_x_left-1;i++)
	{
		drawLine(drawImg, startx-i, starty, endx-i, endy, colorR, colorG, colorB);
	}	

	
	drawLine(drawImg, startx+wid_x_right, starty+1, endx+wid_x_right, endy-1, colorR, colorG, colorB);
	for(i=1;i<=wid_x_right-1;i++)
	{
		drawLine(drawImg, startx+i, starty, endx+i, endy, colorR, colorG, colorB);
	}
	drawLine(drawImg, startx, starty, endx, endy, colorR, colorG, colorB);
...
/////////////
#if 0
	if(startx-wid_x_left>0 &&  starty-wid_y_up>0 && startx+wid_x_right<320 &&  starty+wid_y_dn<240)//未碰到屏幕边框的点进行补全点的操作
	{
//	printf("ldw_Sline[%d,%d][-%d-%d-%d-%d]\n",startx,starty,wid_x_left,wid_y_up,wid_x_right,wid_y_dn);
		for(i=0;i<width;i++)
		{
			for(j=0;j<width;j++)
			{
				//if(i=0&&)
				{
					//drawImg[startx+i-wid_x_left+320*(starty-wid_y_up+j)]=0x001F;		
				}
				//else
				{				
					drawImg[startx+i-wid_x_left+320*(starty-wid_y_up+j)]=colorRGB565;
				}
			}
		}		
	}
	...
	if(endx-wid_x_left>0 &&  endy-wid_y_up>0&& endx+wid_x_right<320 &&  endy+wid_y_dn<240)//未碰到屏幕边框的点进行补全点的操作
	{
	
//	printf("ldw_Eline[%d,%d][-%d-%d-%d-%d]\n",endx,endy,wid_x_left,wid_y_up,wid_x_right,wid_y_dn);
		for(i=0;i<width;i++)
		{
			for(j=0;j<width;j++)
			{
				...
				//else
				{				
		//		printf("ldw_pixele[%d,%d]\n",endx+i-wid_x_left,endy-wid_y_up+j);
					drawImg[endx+i-wid_x_left+320*(endy-wid_y_up+j)]=colorRGB565;
				}
			}
		}		
	}
...
int ap_ppu_init(HANDLE handle)
{
	int i;
	disp1Manager_t *pDisp1= (disp1Manager_t *)handle;
	/* initial ppu register parameter set structure */
	ppu_register_set = (PPU_REGISTER_SETS *) &ppu_register_structure;	
	memset((void *)&ppu_register_structure, 0x0, sizeof(PPU_REGISTER_SETS)); // deyue add 2012-1-12
	...
	gplib_ppu_frame_buffer_mode_set(ppu_register_set, 1, 0);		        // Enable TV/TFT frame buffer mode	
    	//PPU setting
    	gplib_ppu_fb_format_set(ppu_register_set, 0, 0);			            // Set PPU output frame buffer format to YUYV	
	gplib_ppu_vga_mode_set(ppu_register_set, 0);							// Enable VGA mode
	gplib_ppu_free_size_set(ppu_register_set, 0, pDisp1->resolution.width,pDisp1->resolution.height);				// Set display resolution to 640x480    
    
	gplib_ppu_long_burst_set(ppu_register_set, 1);
	gplib_ppu_text_rgba_set(ppu_register_set, 1);
	...
	for(i=0;i<3;i++)
	{
		FrameBuffer[i] = gpChunkMemAlloc(pDisp1->resolution.width*pDisp1->resolution.height*4);
		printf("FrameBuffer[%d] = 0x%x\n",i,FrameBuffer[i]);
        gplib_ppu_frame_buffer_add(ppu_register_set,(int)FrameBuffer[i]);    
	}
}

int ap_ppu_text_load(HANDLE handle)
{
	//get text array buffer
	disp1Manager_t *pDisp1= (disp1Manager_t *)handle;
	TextNumArray1 = (void*)gpChunkMemAlloc(pDisp1->resolution.width*pDisp1->resolution.height*2);  
	TextNumArray2 = (void*)gpChunkMemAlloc(pDisp1->resolution.width*pDisp1->resolution.height*2);  
	//config text attribute
	gplib_ppu_text_compress_disable_set(ppu_register_set, 1);
	
	gplib_ppu_text_enable_set(ppu_register_set, C_PPU_TEXT1, 1);	                 // Enable TEXT	
	gplib_ppu_text_number_array_ptr_set(ppu_register_set, C_PPU_TEXT1, (unsigned int)TextNumArray1);	 // Set TEXT number array address
	gplib_ppu_yuv_type_set(ppu_register_set, 0);								                // Set 32-bit color format to Y1UY0V
	gplib_ppu_text_bitmap_mode_set(ppu_register_set, C_PPU_TEXT1, 1);			     // Enable bitmap mode
	gplib_ppu_text_color_set(ppu_register_set, C_PPU_TEXT1, 1, 3);//3 	vyuy				     // Set TEXT color to YUYV
	gplib_ppu_text_size_set(ppu_register_set, C_PPU_TEXT1, 5);			             // Set TEXT size to 1024x1024	
	gplib_ppu_text_attribute_source_select(ppu_register_set, C_PPU_TEXT1, 1);  

	gplib_ppu_text_enable_set(ppu_register_set, C_PPU_TEXT2, 1);	                 // Enable TEXT	
	gplib_ppu_text_number_array_ptr_set(ppu_register_set, C_PPU_TEXT2, (unsigned int)TextNumArray2);	 // Set TEXT number array address
	gplib_ppu_yuv_type_set(ppu_register_set, 0);								                // Set 32-bit color format to Y1UY0V
	gplib_ppu_text_bitmap_mode_set(ppu_register_set, C_PPU_TEXT2, 1);			     // Enable bitmap mode
	gplib_ppu_text_color_set(ppu_register_set, C_PPU_TEXT2, 1, 1);//3 	vyuy				     // Set TEXT color to YUYV
	gplib_ppu_text_size_set(ppu_register_set, C_PPU_TEXT2, 5);			             // Set TEXT size to 1024x1024	
	gplib_ppu_text_attribute_source_select(ppu_register_set, C_PPU_TEXT2, 1);  	
	//setup text content by jpeg file
	gplib_ppu_text_calculate_number_array(ppu_register_set, C_PPU_TEXT2, pDisp1->resolution.width,pDisp1->resolution.height, (unsigned int)disp1GetFramebuffer(handle));	// Calculate Number array  
...
{
	gplib_ppu_text_color_set(ppu_register_set, C_PPU_TEXT1, 1, 1);//3 	vyuy				     // Set TEXT color to YUYV
}
...
{
	disp1Manager_t *pDisp1;

	pDisp1 = (disp1Manager_t *)malloc(sizeof(disp1Manager_t));
	memset(pDisp1, 0, sizeof(disp1Manager_t));
	pDisp1->layer = layer;
	
	/* Opening the device disp1Dev */
	printf("##################open disp1####################\n");
	pDisp1->fdDisp1 = open("/dev/disp1", O_RDWR);
	ioctl(pDisp1->fdDisp1, DISPIO_SET_INITIAL, 0);
  ioctl(pDisp1->fdDisp1, DISPIO_GET_PANEL_RESOLUTION, &pDisp1->resolution);
  printf("Disp1 : width = %d, height = %d\n",pDisp1->resolution.width,pDisp1->resolution.height);

	/* Allocate framebuffer */
	int bufferSize = pDisp1->resolution.width * pDisp1->resolution.height * 2;
	pDisp1->fdMem = open("/dev/chunkmem", O_RDWR);
	pDisp1->memBlock.size = bufferSize * 2;
	ioctl(pDisp1->fdMem, CHUNK_MEM_ALLOC, (unsigned long)&pDisp1->memBlock);

	/* Setup bitmap double buffer */
	for (int i = 0; i < 2; i++) {
		gp_bitmap_t *fb = &pDisp1->fb[i];
		fb->width = pDisp1->resolution.width;
		fb->height = pDisp1->resolution.height;
		fb->bpl = fb->width * 2;
		fb->type = SP_BITMAP_RGB565;
		fb->pData = pDisp1->memBlock.addr + i * bufferSize;
	}
	pDisp1->fbIndex = 0;
	printf("pDisp1->fb[pDisp1->fbIndex].pData = 0x%x\n",pDisp1->fb[pDisp1->fbIndex].pData);
	memset(pDisp1->fb[pDisp1->fbIndex].pData, 0, bufferSize);
		
	ioctl(pDisp1->fdDisp1, DISPIO_SET_PRI_BITMAP, &pDisp1->fb[pDisp1->fbIndex]);
	ioctl(pDisp1->fdDisp1, DISPIO_SET_PRI_ENABLE, 1);

	*pHandle = (HANDLE)pDisp1;
...
)
{
	disp1Manager_t *pDisp1= (disp1Manager_t *)handle;
	printf("##################close disp1####################\n");
	ioctl(pDisp1->fdDisp1, DISPIO_SET_PRI_ENABLE, 0);
	close(pDisp1->fdDisp1);
	ioctl(pDisp1->fdMem, CHUNK_MEM_FREE, &pDisp1->memBlock);
	close(pDisp1->fdMem);	
	free(pDisp1);
...
)
{
	...
	resolution->width = pDisp1->resolution.width;
	resolution->height = pDisp1->resolution.height;
}
...
{
	disp1Manager_t *pDisp1 = (disp1Manager_t *)handle;
	int temp = pDisp1->fbIndex^1;
	return (UINT8*) pDisp1->fb[temp].pData;
}
...
{
	disp1Manager_t *pDisp1 = (disp1Manager_t *)handle;
	pDisp1->fbIndex = pDisp1->fbIndex^1;
	printf("Draw End !!!\n");
	return;
}
void
disp1Flip(
	HANDLE handle,UINT32 *addr
)
{
	disp1Manager_t *pDisp1 = (disp1Manager_t *)handle;
	disp1Manager_t temp;
	...
	if(addr != 0)
	{
		gplib_ppu_text_calculate_number_array(ppu_register_set, C_PPU_TEXT1, pDisp1->resolution.width, pDisp1->resolution.height, addr);	// Calculate Number array  
	}
	else
	{
		gplib_ppu_text_enable_set(ppu_register_set, C_PPU_TEXT1, 0);	      
	}
	gplib_ppu_text_calculate_number_array(ppu_register_set, C_PPU_TEXT2, pDisp1->resolution.width, pDisp1->resolution.height, (UINT8*) pDisp1->fb[pDisp1->fbIndex].pData);	// Calculate Number array  
	gplib_ppu_go_and_wait_done(ppu_register_set);	
	while(!(display_buffer = gplib_ppu_frame_buffer_get()));	
	//ioctl(pDisp1->fdDisp1, DISPIO_CHANGE_PRI_BITMAP_BUF, display_buffer);
	//printf("display_buffer = 0x%x\n",display_buffer);
	temp.fb[0].pData = display_buffer;
	temp.fb->width = pDisp1->resolution.width;
	temp.fb->height = pDisp1->resolution.height;
	temp.fb->bpl = temp.fb->width * 2;
	temp.fb->type = SP_BITMAP_RGB565;
	//pDisp1->fbIndex = pDisp1->fbIndex ^ 1;	
	ioctl(pDisp1->fdDisp1, DISPIO_CHANGE_PRI_BITMAP_BUF, &temp.fb[0]);	
	ioctl(pDisp1->fdDisp1, DISPIO_SET_UPDATE, 0);
	ioctl(pDisp1->fdDisp1, DISPIO_WAIT_FRAME_END, 0);
}
