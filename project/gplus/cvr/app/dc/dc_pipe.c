...
void dc_pipemsg_send(unsigned int msgId, unsigned int len, unsigned char *cmddata)
{
	...
	dcCmdPacket_t *pCmdPacket = NULL;
	int writesize = sizeof(dcCmdPacket_t) + len;
	int ret = 0;
	
	pthread_mutex_lock(&mutex_pipewrite);
	memset(pCmdBuf, 0, sizeof(pCmdBuf));

	if (len > (sizeof(pCmdBuf)- sizeof(dcCmdPacket_t)))
	{
		DEBUG("datasize too large\n");
		len = (sizeof(pCmdBuf)-sizeof(dcCmdPacket_t));
	}	

	pData = (unsigned char *)pCmdBuf;
	pCmdPacket = (dcCmdPacket_t *)pData;
	pCmdPacket->infoID = msgId;
	pCmdPacket->dataSize = len;
	memcpy(pData + sizeof(dcCmdPacket_t), cmddata, len);
...
int dc_pipemsg_receive(unsigned int *msgId, void **para)
{
	...
	{
		pCmdPacket = (dcCmdPacket_t *)pCmdBuf;
		pData = (unsigned char *)(((unsigned char *)pCmdBuf) + sizeof(dcCmdPacket_t));
		dataSize = sizeof(dcCmdPacket_t);
...
void dc_response_cmd(unsigned int msgId)
{
	dc_pipemsg_send(msgId, 0, NULL);
}