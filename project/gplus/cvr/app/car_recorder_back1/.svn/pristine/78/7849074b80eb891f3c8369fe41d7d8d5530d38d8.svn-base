...
#include <stdio.h>
#include <stdlib.h>
#define __MODULE__          "AP_State_Config"
...
void statusScanThread( void)
{
	...
	{
		printf(" insert or plugout sdc,Enter Power Off Flow:%s\n",Memory_path);
		dv_set.sd_check = ret;
		if(dv_set.dv_ctrl == 1)
		{
	...
	do{
		...
		if(j%5 == 0)
		{
			{	
				...
				if((int)j >= 1000)		
				b = "\b\b\b\b\b";	
				fprintf(stderr,"\b\b%sj= %d", b, (int)j);		
			}
			name_len ++;
			if(name_len > 4)
				name_len = 0;
	...
	for(i=0;i<loop_num;i++)
	{
		...
		if((int)old_value != (int)value)
		{
			...
			{	
				...
				if((int)value >= 1000)		
				b = "\b\b\b\b\b";	
				fprintf(stderr,"\b%s %d%%", b, (int)value);		
			}
			//fprintf(stderr,"\b\b\b\b %3d",(int)value);
		}
		spi_addr[0] = 0x06;		
		ioctl(spi_fd, SPI_IOCTL_CS_ENABLE, 0);		
		write(spi_fd, spi_addr, 1);		
