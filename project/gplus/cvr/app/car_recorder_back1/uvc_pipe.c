...
int uvc_pipe_init(void)
{
	int err = 0;
	unlink(UVC_FIFO1);
	if ((mkfifo(UVC_FIFO1, O_CREAT | O_EXCL) < 0) && (errno != EEXIST)) {
		printf("can't create fifo server\n");
		printf("error: %s!\n", strerror(errno));
		return -1;
	}
	unlink(UVC_FIFO2);
	if ((mkfifo(UVC_FIFO2, O_CREAT | O_EXCL) < 0) && (errno != EEXIST)) {
		printf("can't create fifoserver\n");
		printf("error: %s!\n", strerror(errno));
		return -1;
	}

	g_CmdIn = pipe_init(UVC_FIFO1);
	if (!g_CmdIn)
		goto init_fail;

	g_CmdOut = pipe_init(UVC_FIFO2);
	if (!g_CmdOut)
		goto init_fail;
