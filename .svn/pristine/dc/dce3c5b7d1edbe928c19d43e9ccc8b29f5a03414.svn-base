...
static int A_Sensor_Init(unsigned char *status,ti2c_set_value_t *g_ti2c_handle)
{
	...
	value =0;
	A_sensor_read(DMT_ARD07_CTRL_REG_6,&value,g_ti2c_handle);
	if((value) && (value != 0xff))
	{
		printk("baiqi *********** gsensor power on:0x%x\n",value);	
		*status = 1;	
	}	
	else	
	{		
		printk("baiqi *********** no gsensor power on\n");
		*status =0;
	}
...
void A_sensor_uninit(UINT32 level,struct ti2c_set_value_s *g_ti2c_handle) //for power off.
{
	...
	A_sensor_read(0x53,&data,g_ti2c_handle);
	A_sensor_write(DMT_ARD07_CTRL_REG_1, 0x47,g_ti2c_handle); // normal mode, enable interrupt,data rate 342HZ
	A_sensor_write(DMT_ARD07_CTRL_REG_2,0x24,g_ti2c_handle);// 0x24  +-2g data low filter,int 1 source hight filter
	A_sensor_write(DMT_ARD07_CTRL_REG_3,0x00,g_ti2c_handle);
	A_sensor_write(DMT_ARD07_CTRL_REG_4,0x64,g_ti2c_handle); //0x64 ativity high ,latch  SRC1, SRC2 latch disable, int pin to SRC1
	A_sensor_write(DMT_ARD07_CTRL_REG_9,0x00,g_ti2c_handle);//auto  Awake function disable
	...
	switch(level)
	{
		case 3://low	
			...
			break;
		case 2: 
			A_sensor_write(DMT_ARD07_CTRL_REG_7, DMT_ARD07_MID,g_ti2c_handle);
			break;
		case 1:
			A_sensor_write(DMT_ARD07_CTRL_REG_7,DMT_ARD07_HIGH,g_ti2c_handle);
			break;
	}
	A_sensor_write(DMT_ARD07_CTRL_REG_8,0x04,g_ti2c_handle);
	A_sensor_write(DMT_ARD07_CTRL_REG_5,0xea,g_ti2c_handle);
	A_sensor_read(DMT_ARD07_CTRL_REG_4, &data,g_ti2c_handle);
	
	printk("end reg[0x47]=%x\n",data);
}
#else
static int A_Sensor_Init(unsigned char *status,int *g_ti2c_handle)
{
	...
	if((value) && (value != 0xff))
	{
		printk("baiqi *********** gsensor power on:%d\n",value);	
		*status = 1;	
	}	
	else	
	{		
	...
	A_sensor_write(DMT_ARD07_CTRL_REG_3,0x00,g_ti2c_handle);
	//A_sensor_read(DMT_ARD07_CTRL_REG_3,&value,g_ti2c_handle);
	A_sensor_write(DMT_ARD07_CTRL_REG_4,0x6c,g_ti2c_handle); //0x4c 0x0c data ready int,activity high
	//A_sensor_read(DMT_ARD07_CTRL_REG_4,&value,g_ti2c_handle);
...
void A_sensor_uninit(UINT32 level, int *g_ti2c_handle) //for power off.
{
	...
	A_sensor_read(0x53,&data,g_ti2c_handle);
	A_sensor_write(DMT_ARD07_CTRL_REG_1, 0x27,g_ti2c_handle); // normal mode, enable interrupt,data rate 342HZ
	A_sensor_write(DMT_ARD07_CTRL_REG_2,0x24,g_ti2c_handle);// +-2g data low filter,int 1 source hight filter
	A_sensor_write(DMT_ARD07_CTRL_REG_3,0x00,g_ti2c_handle);
	A_sensor_write(DMT_ARD07_CTRL_REG_4,0x24,g_ti2c_handle); //ativity high ,latch  SRC1, SRC2 latch disable, int pin to SRC1
	A_sensor_write(DMT_ARD07_CTRL_REG_9,0x00,g_ti2c_handle);//auto  Awake function disable
...
{	
	...
	{	
		case ASENSOR_IOCTL_GET_INT_ACTIVE:	
				...
				A_sensor_read(0x4b,&temp,&ptr_asensor->g_ti2c_handle);
				//printk("[%s][%d]: 0x4b value is [0x%x]\r\n",__FUNCTION__,__LINE__,temp);
				if((temp &0x40)) 
				{ 
					//active int flag
					//A_sensor_write(DMT_ARD07_CTRL_REG_7,DMT_ARD07_LOW,&ptr_asensor->g_ti2c_handle);
					temp = 1;
			...
			switch(arg)
				{
					...
					break;
					case 2://mid
						A_sensor_write(DMT_ARD07_CTRL_REG_7,DMT_ARD07_MID,&ptr_asensor->g_ti2c_handle);
					break;
					case 1://high
						A_sensor_write(DMT_ARD07_CTRL_REG_7,DMT_ARD07_HIGH,&ptr_asensor->g_ti2c_handle);
					break;
				}
			break;
		case ASENSOR_IOCTL_PARK_MODE:
			printk("[%s][%d]:level is [%ld]\r\n",__FUNCTION__,__LINE__,arg);
				A_sensor_uninit(arg, &ptr_asensor->g_ti2c_handle);
			break;
		case ASENSOR_IOCTL_PARK_MODE_INT:
				if(copy_to_user((void __user *)arg,&ptr_asensor->g_sensor_power_on,sizeof(int)))			
				return -EFAULT;		
			break;
		case ASENSOR_IOCTL_GET_G_VALUE:
			A_sensor_read(0x41,(unsigned char*)&x_out,&ptr_asensor->g_ti2c_handle);
			A_sensor_read(0x42,(unsigned char*)&y_out,&ptr_asensor->g_ti2c_handle);
			A_sensor_read(0x43,(unsigned char*)&z_out,&ptr_asensor->g_ti2c_handle);
			x_out=x_out+128;//x=0, -2g;	m=128,0g   m=127,2g[actually is (2-1/64)g]-->szk add
			y_out=y_out+128;
			z_out=z_out+128;			
			g_value=x_out*256*256+y_out*256+z_out;
