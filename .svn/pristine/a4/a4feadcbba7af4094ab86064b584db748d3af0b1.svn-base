...
)
{
	...
	g_save_Mux->set(storageHandle->hdMux, MUX_MAX_SIZE, freeDiskSize);
	...
	while (1) {
		...
		if(msg->cmd == STORAGE_CMD_WRITE || msg->cmd == STORAGE_CMD_WRITE_AUDIO) {
			...
			if (storageHandle && storageHandle->hdMux) {
				if (isAudio)
				{
					MuxPkt.data = ((audio_buffer_t*)(msg->buffer))->frame->pAddr;
				...
				{
					MuxPkt.data = ((video_buffer_t*)(msg->buffer))->frame->pFrameAddr;
					MuxPkt.size = msg->nBytes;
					MuxPkt.pts = (storageHandle->last_pts == 0) ? 1000/storageHandle->framerate : msg->pts - storageHandle->last_pts;
					storageHandle->last_pts = msg->pts;
					MuxPkt.keyFrame = msg->frameType;
					if(storageHandle->hdMux)
