...
macro define
***********************************/
#define IT7260_address0 (0x8c)
#define IT7260_address1 (0x6a)
#define IT7260_address2 (0xdc)
#define IT7260_address3 (0x2e)

#define TI2C_ADDR IT7260_address0
#define TI2C_CLK (200)
...
static int touch_event_handler( void *arg)
{
	...
	#endif
		ret = tpd_i2c_read(ts,0xE0, &pucPoint[0], 14);
		if (ret>=0) {

			// gesture
				if (pucPoint[0] & 0xF0) {
					printk("[mtk-tpd] it's a button/gesture\n");
				}
				// palm
				else if( pucPoint[1] & 0x01 ) {
					printk("[mtk-tpd] it's a palm\n");
				}
				// no more data
				...
				// 3 fingers
					...
					printk("[mtk-tpd] we don't support three fingers\n");
				}else{
					// finger 1
					if (pucPoint[0] & 0x01) {
						//char pressure_point;
						...
						DEBUG("[mtk-tpd] input Read_Point1 x=%d y=%d\n",xraw,yraw);
						//tpd_calibrate(&xraw, &yraw);			
						x[0] =xraw;
						y[0] = yraw;
						report_data(x[0], y[0], 70, 0);
					}
					...
					if (pucPoint[0] & 0x02) {
						...
						DEBUG("[mtk-tpd] input Read_Point2 x=%d y=%d\n",xraw,yraw);
						//tpd_calibrate(&xraw, &yraw);
						x[1] = xraw;
						y[1] = yraw;
						report_data(x[1], y[1], 70, 1);
					}
					input_sync(ts->dev);
				}
			}else{
				printk("[mtk-tpd] i2c read communcate error in getting pixels : 0x%x\n", ret);
			}
...
static int __init gp_tp_probe(struct platform_device *pdev)
{
	...
	ts.i2c_handle->pDeviceString = "Touch_IT7260";
	ts.i2c_handle->slaveAddrMode = TI2C_NORMAL_SLAVEADDR_8BITS;
...
static int gp_tp_resume(struct platform_device *pdev)
{
	...
	gp_board_touch_t *touch_config = NULL;
	int intidx;
	printk("IT7260 call resume\n");

	for( i = 0 ; i < 5 ; i++ ){
		buffer = 0xFF;
		ret = tpd_i2c_read(&ts,0x80, &buffer,1 );
		if( buffer != 0xFF )
			break;
	}
