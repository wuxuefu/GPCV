...
#include <stdint.h>
#include "typedef.h"
//#include "GpSystemFuntion/video/inlcude/typedef.h"
...
enum {
	PLAYER_CMD_SET_CONTAINER_TYPE = 0,
	PLAYER_CMD_SET_CODEC_TYPE,
	PLAYER_CMD_SET_URL,
	PLAYER_CMD_SET_TIMESTAMP,
	PLAYER_CMD_SET_VID_OUT,
	PLAYER_CMD_SET_VID_MODE,
	
	PLAYER_CMD_SET_VID_LAYER,  /*6*/
	PLAYER_CMD_SET_VID_COLOR,
	PLAYER_CMD_SET_VID_WIDTH,
	PLAYER_CMD_SET_VID_HEIGHT,
	PLAYER_CMD_SET_VID_RECT,

	PLAYER_CMD_SET_VID_WINDOW,  /*11*/
	PLAYER_CMD_SET_VID_ASPECT,
	PLAYER_CMD_SET_VID_DECRYPT,
	PLAYER_CMD_SET_AUD_OUT,
	PLAYER_CMD_SET_AUD_VOLUME,
	
	PLAYER_CMD_SET_SUB_LANGUAGE,  /*16*/
	PLAYER_CMD_SET_SUB_INDEX,
	PLAYER_CMD_SET_PLAY_MSG,
	PLAYER_CMD_SET_PLAY_RENDER,

	PLAYER_CMD_GET_VERSION,              /*20*/
	PLAYER_CMD_GET_CONTAINER_TYPE,
	PLAYER_CMD_GET_VID_INFO,
	PLAYER_CMD_GET_AUD_INFO,
	PLAYER_CMD_GET_SUB_INFO,
	PLAYER_CMD_GET_DURATION,
	
	PLAYER_CMD_GET_SEEK_MODE,       /*26*/
	PLAYER_CMD_GET_PLAY_TIME,
	PLAYER_CMD_GET_PLAY_STATE,
	PLAYER_CMD_GET_SUB_COUNT,
	PLAYER_CMD_SET_SUB_TTF,

	PLAYER_CMD_STOP_EX,                   /*30*/
	PLAYER_CMD_SEEK_EX,
	PLAYER_CMD_RESUME_EX,
	PLAYER_CMD_PAUSE_EX,
	PLAYER_CMD_PLAY_EX,
	PLAYER_CMD_THUMBNAIL_EX,
	PLAYER_CMD_CLOSE_EX,
	PLAYER_CMD_MAX_EX
};
...
typedef int (*VideoEventHandler_t)(unsigned int event, unsigned int param);

int videoPlayerSetAudVolume( int volume );

int videoRegisterEventhandler(VideoEventHandler_t handler);

// play the file
int PlayerVideoPlay(const char *videoPath, const char *filePath);

int VideoPlayerOpen( void );


int VideoPlayerClose( void );

void SetResumeTOCmdPacket( void );

void SetPauseToCmdPacket( void );

void SetGetInfoToCmdPacket( void );

int SetStopValueToCmdPacket( void );

int SetSeekValueToCmdPacket(unsigned long seekTime);

int SetVolValueToCmdPacket( int volume );

int GetVolValueToCmdPacket( void );

void  threadRead( void );

UINT32 VideoGetTotalTime(void) ;

unsigned long GetCurTimeTOCmdPacket( void );

UINT32 GetVideoStatus( void );

UINT32 videoGetBitmap(gp_rect_t * pRect);

UINT32 videoSetBitmap(gp_rect_t * pRect, UINT32 aspect);

UINT32 SetSlowCmdPacket( UINT32 slowValue, int isPlus );
int getVideoToCmdPacket( void );

int videoPlayerSetAudOutOss( void );
int videoPlayerSetAudOutMixer( void );
int videoPlayerSetTimeStamp( UINT32 ms );
int videoPlayerSetVideoAspect( UINT32 asp );
int videoPlayerGetSeekMode( void );

unsigned long VideoGetCurTime2(void);
