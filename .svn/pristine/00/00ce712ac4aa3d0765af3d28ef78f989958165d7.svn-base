...
)
{
	...
	uname(&uts);
	memset(&module_order, 0, sizeof(module_order));
	snprintf(module_order, sizeof(module_order), "%s/%s/modules.order", MODULE_PATH, uts.release);
...
)
{
	...
	if (filename[0] != '/') {
		...
		snprintf(module_path, sizeof(module_path), "%s/%s/%s", MODULE_PATH, uts.release, subpath);
