...
#define __MODULE__          "Board_Config"
...
)
{
	...
	int handle;
	gpio_content_t ctx;
    printk("\033[31m%s enable:%d Channel:%d\033[0m\r\n",__func__,enable,sd0_set_power_channel);
	if ( channel == 0xff ) {
		return -1;	/* No Function */
	}
...
)
{
	...
	int handle;
	gpio_content_t ctx;
    printk("\033[31m%s enable:%d Channel:%d\033[0m\r\n",__func__,enable,sd1_set_power_channel);
...
)
{
	...
	int handle;
	gpio_content_t ctx;
    printk("\033[31m%s enable:%d Channel:%d\033[0m\r\n",__func__,enable,sd2_set_power_channel);
...
)
{
	...
	pConfig = gp_board_get_config("board", gp_board_t);//gp_board_config_t
