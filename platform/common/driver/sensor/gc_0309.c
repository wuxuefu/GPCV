...
)
{
...
#elif (I2C_MODE == HW_TI2C)
	g_ti2c_handle.pDeviceString = "GC0309";		
	...
	if (gp_ti2c_bus_request(&g_ti2c_handle) != 0) {
		printk("GC0309 ti2c request failed\n");
		return -1;
	}
...
)
{
	...
	sensor_write(0xfe, 0x00);	// set page0
	sensor_write(0x1a, 0x16);
	...
	sensor_write(0xe2, 0x00);
	sensor_write(0xe3, 0x64);
	sensor_write(0xe4, 0x02);
	sensor_write(0xe5, 0x58);
	sensor_write(0xe6, 0x02);
	sensor_write(0xe7, 0x58);
	sensor_write(0xe8, 0x02);
	sensor_write(0xe9, 0x58);
	sensor_write(0xea, 0x09);
	sensor_write(0xeb, 0xc4);
	sensor_write(0xd2, 0x90);	// Open AEC
	...
	sensor_write(0xc0, 0x00);	//Y_gamma
	sensor_write(0xc1, 0x0B);
	sensor_write(0xc2, 0x15);
	sensor_write(0xc3, 0x27);
	sensor_write(0xc4, 0x39);
	sensor_write(0xc5, 0x49);
	sensor_write(0xc6, 0x5A);
	sensor_write(0xc7, 0x6A);
	sensor_write(0xc8, 0x89);
	sensor_write(0xc9, 0xA8);
	sensor_write(0xca, 0xC6);
	sensor_write(0xcb, 0xE3);
	sensor_write(0xcc, 0xFF);
	sensor_write(0xf0, 0x02);	// ABS_t
	sensor_write(0xf1, 0x01);
	sensor_write(0xf2, 0x00);	// 0x04
	sensor_write(0xf3, 0x30); 
	sensor_write(0xf7, 0x04);	// Measure Window
	sensor_write(0xf8, 0x02);
	sensor_write(0xf9, 0x9f);
	sensor_write(0xfa, 0x78);
	sensor_write(0xfe, 0x01);
	sensor_write(0x00, 0xf5);	// AWB_p
	sensor_write(0x01, 0x0a);
	sensor_write(0x02, 0x1a);
	sensor_write(0x0a, 0xa0);
	sensor_write(0x0b, 0x64);
	sensor_write(0x0c, 0x08);
	sensor_write(0x0e, 0x4c);
	sensor_write(0x0f, 0x39);
	sensor_write(0x11, 0x3f);
	sensor_write(0x13, 0x11);
	sensor_write(0x14, 0x40);
	sensor_write(0x15, 0x40);
	...
	sensor_write(0x23, 0x00);	// effect normal mode
	sensor_write(0x2d, 0x0a);
	sensor_write(0x20, 0xff);
	sensor_write(0xd2, 0x90);
	sensor_write(0x73, 0x00);
	sensor_write(0x77, 0x38);
	sensor_write(0xb3, 0x40);
	sensor_write(0xb4, 0x80);
	sensor_write(0xba, 0x00);
	sensor_write(0xbb, 0x00);
	sensor_write(0x28, 0x00);	// clk  24M
	//sensor_write(0x28,0x11);	// clk  12M
	msleep(10);
	return sensor_write(0x14, 0x10);	// normal
	//sensor_write(0x14, 0x13); 180
}
...
)
{
	...
	case V4L2_CID_POWER_LINE_FREQUENCY:
		...
		if(ctrl->value == V4L2_CID_POWER_LINE_FREQUENCY_DISABLED) {
			...
			nRet = sensor_write(0xe3, 0x96);	// anti-flicker step [7:0]
			nRet = sensor_write(0xe4, 0x02);	// exp level 1  16.67fps
			nRet = sensor_write(0xe5, 0x58); 
			nRet = sensor_write(0xe6, 0x02);	// exp level 2  12.5fps
			nRet = sensor_write(0xe7, 0x58); 
			nRet = sensor_write(0xe8, 0x02);	// exp level 3  8.33fps
			nRet = sensor_write(0xe9, 0x58); 
			nRet = sensor_write(0xea, 0x0b);	// exp level 4  4.00fps
			nRet = sensor_write(0xeb, 0xb8);
		}else if(ctrl->value == V4L2_CID_POWER_LINE_FREQUENCY_60HZ) {
			nRet = sensor_write(0x01, 0x6a); 	
			nRet = sensor_write(0x02, 0x0c); 
			nRet = sensor_write(0x0f, 0x00);
			//write_cmos_sensor(0x03, 0x01); 	
			...
			nRet = sensor_write(0xe4, 0x01);	//exp level 1  15.00fps
			nRet = sensor_write(0xe5, 0xf4); 
			nRet = sensor_write(0xe6, 0x01);	//exp level 2  10.00fps
			nRet = sensor_write(0xe7, 0xf4); 
			nRet = sensor_write(0xe8, 0x01);	//exp level 3  7.50fps
			nRet = sensor_write(0xe9, 0xf4); 
			nRet = sensor_write(0xea, 0x09);	//exp level 4  4.00fps
			nRet = sensor_write(0xeb, 0xc4);
		}else{
			return -EINVAL;
		}
		break;
...
{
	if(sensor_i2c_open(GC0309_ID, 100) < 0) {
		printk(KERN_WARNING "i2cReqFail\n");
		return -1;
	}

	printk(KERN_WARNING "ModuleInit: gc0309 \n");
	g_sensor_dev.fmt = &g_fmt_table[0];
	v4l2_subdev_init(&(g_sensor_dev.sd), &sensor_ops);
	strcpy(g_sensor_dev.sd.name, "sensor_gc0309");
...
MODULE_DESCRIPTION("Generalplus gc0309 Driver");
MODULE_LICENSE("GPL");
MODULE_VERSION("1.0");
