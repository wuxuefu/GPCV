...
static int32_t lcd_resume_post(void);
...
)
{
	...
	/* close all layers(primary, osd) before disable lcd clock */
	disp_close_all_layers();
	printk("After close all layers mdelay 100 to prevent stop clock crash\n");
	mdelay(100);
...
{
	/* 
	 * sleep 80ms to wait for display control start work and then turn on backlight
	 * to show logo more quickly when fastboot
	 */
