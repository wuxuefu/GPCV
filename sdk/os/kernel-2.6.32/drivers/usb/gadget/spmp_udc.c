	...
	if( req->req.length == 31 ) {
		...
		while(1) {
			...
			if( ret != 0 && (!spmp_vbus_detect() || isResetInt || mscReset || gpHalUsbGetNoneDetectRemove() || (!gpHalUsbSlaveSwConnectGet()) || (usbExitSaveRemoveConfig && gpHalUsbHostSafyRemoved()))) {
				printk("CBW[1]v[%d]r[%d]mr[%d]sw[%d]sm c[%d]sm[%d]\n", 
					   spmp_vbus_detect(), isResetInt, mscReset, !gpHalUsbSlaveSwConnectGet(), usbExitSaveRemoveConfig, gpHalUsbHostSafyRemoved());
...
static irqreturn_t spmp_udc_irq(int dummy, void *_dev)
{
	...
	/*UDC_IF_OFST [0:23] is the same with UDC_LLCIF_OFST. But we should write UDC_LLCIF_OFST for clear. 
	  allenchang 20110506*/
	//udc_irq_flags = udc_read(UDC_LLCIF_OFST);
	if(udc_ctl_irq_flags & udc_irq_en1 & CIS_FDISCONN_IF )
	{
