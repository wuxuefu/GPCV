...
static STRUCT_MIDI_SPU spu_struct, spu_struct_temp;
static STRUCT_DRM_SPU drm_struct;

static int spu_dev_opened;
...
UINT32 midi_buffer_Kaddr;
UINT8* ggg_pMIDI_DataPointer;

static UINT32 T_InstrumentSectionPhyAddr[500];
static UINT32 *T_InstrumentSectionKAddr[500];
static UINT32 T_DrumPhyAddr[128];
static UINT32 *T_DrumKAddr[128];
...
static void spu_suspend1(void);
static void spu_resume1(void);
#endif
...
static void spu_device_device_release(struct device *dev)
{
	printk("remove spu_device_device_release ok \n");
}

static struct platform_device spu_device = {
	.name	= "gp-spu",
	.id	= -1,
	.dev	= {
		.release = spu_device_device_release,
	}
};

/**                                                                         
 * @brief spu driver define                                               
 */                                                                         
 ...
 **************************************************************************/
int spu_dac_freq_set(int freq, int type)
{
	int ratio=0, reg;
...
void spu_dac_power_ctrl(int enable)
{
	if(enable) {
		/* enable related clock: i2s, dac, apbdma */
		gpHalI2sClkEn(enable);
		gpHalDacClkEn(enable);
//		gpHalApbdma1ClkEn(enable);
				
		/* if apbdma has opened in 2nd boot loader, reset it */
//		gpHalApbdma1Rst(0);
		
		/* give default freqency 44100 */
		spu_dac_freq_set(48000, 1);

	//	gpHalLinoutVolset();
	//	gpHalAdcSetGND();
	//	gpHalHdphnVolset(0x1f, 0x1f);		//mute
	//	udelay(100);
/*
		gpHalHdphnEn(enable);
	//	gpHalAdcSetVref();
		gpHalVrefCtrl(enable);
...
//	unsigned int *pAddr;
/*
		SPU_DBG("*****************************************before addr mapping = 0x%x \n",pAddr);	
		temp = gp_user_va_to_pa(pAddr);
		pAddr = (unsigned int *)gp_chunk_va(temp);
		SPU_DBG("*****************************************after addr mapping = 0x%x \n",pAddr);		
*/	
//	SPU_DBG("ALP addr = 0x%x \n", pAddr);
...
UINT16 get_a_word(void)
{
	...
	temp = *ggg_pMIDI_DataPointer++;//low 8 bit
	spu_struct_p->pMIDI_DataPointer++;
	if( (UINT32)ggg_pMIDI_DataPointer >= ((UINT32)midi_buffer_Kaddr + MIDI_RING_BUFFER_SIZE) )
	{
		ggg_pMIDI_DataPointer = (UINT8 *)midi_buffer_Kaddr;
		spu_struct_p->pMIDI_DataPointer = (UINT8 *) spu_struct_p->midi_ring_buffer_addr;
		//SPU_DBG("get_a_wort = 0x%x \n",ggg_pMIDI_DataPointer);
	}
	temp1 = *ggg_pMIDI_DataPointer++;//high 8 bit
	spu_struct_p->pMIDI_DataPointer++;
	if( (UINT32)ggg_pMIDI_DataPointer >= ((UINT32)midi_buffer_Kaddr + MIDI_RING_BUFFER_SIZE) )
	{		
		ggg_pMIDI_DataPointer = (UINT8 *)midi_buffer_Kaddr;
...
//ggg			pAddr = (unsigned int *)spu_struct_p->T_DrumAddr;
	...
	//		R_BasePitch = *(pAddr + SectionIndex) >> 8;
			pAddr = (unsigned int *)(T_DrumKAddr + (spu_struct_p->MIDI_Event).NoteEvent.Pitch);
			phyAddr = (unsigned int)(T_DrumPhyAddr [(spu_struct_p->MIDI_Event).NoteEvent.Pitch]);//20110415
...
//			printk(KERN_WARNING "drum value = %x \r\n", value);
		...
		else	// not drum
		{
			...
			R_BasePitch = *(pAddr + SectionIndex) >> 8;
			pAddr = (unsigned int *)(T_InstrumentSectionKAddr + SectionIndex);
			phyAddr = T_InstrumentSectionPhyAddr[SectionIndex];
...
void ProcessEndEvent(void)
{
	...
	if(spu_struct_p->MIDI_PlayFlag & STS_MIDI_REPEAT_ON)
	{
		...
		spu_struct_p->pMIDI_DataPointer = (UINT8*)(midi_buffer_Kaddr + spu_struct_p->static_midi_ring_buffer_ri);
		gpHalSPU_Set_BeatCounter(0x0000);
		gpHalSPU_Clear_BeatIRQ_Flag();
		gpHalSPU_Enable_BeatIRQ();	
	}
...
void SPU_PlayPCM_NoEnv_FixCH(UINT32 *pAddr, UINT32 phyAddr, UINT8 uiPan, UINT8 uiVelocity, UINT8 uiSPUChannel)//20110503
{
	...
	//printk("SPU_PlayPCM_NoEnv_FixCH() entry\n");//20110705 test
	...
	if((spu_struct_p->ALP_Header).WaveType & 0x0040)				// ADPCM mode
	{
		...
		if((spu_struct_p->ALP_Header).WaveType & 0x0080)			// ADPCM36 mode
		{
			...
			//gpHalSPU_SetToneColorMode(2, uiSPUChannel);		// HW auto-repeat mode //20110719 test
			//gpHalSPU_SetLoopAddress((spu_struct_p->uiAddr), uiSPUChannel);//20110719
			//printk("adpcm36\n");//20110718 test
		}
		else
		{
		...
		gpHalSPU_SetWaveData(0x8000, uiSPUChannel);		// tone color first sample//20110718
	}
	else	// PCM mode
	{
		//printk("pcm\n");//20110718 test
		...
		gpHalSPU_SetToneColorMode(1, uiSPUChannel);			// HW auto-end mode
		gpHalSPU_SetWaveData(*(pAddr + 10), uiSPUChannel);		// tone color first sample //20110718
		//gpHalSPU_SetLoopAddress((spu_struct_p->uiAddr), uiSPUChannel);//20110719
	}
	gpHalSPU_SetWaveData_0(0x8000, uiSPUChannel);
	//printk("first sample is 0x%x\n",*(pAddr + 10));//20110718 test
	//gpHalSPU_SetWaveData(*(pAddr + 10), uiSPUChannel);		// tone color first sample//20110718 mask
	
	//gpHalSPU_SetLoopAddress(0x0000, uiSPUChannel);//20110719 mask
	gpHalSPU_SetLoopAddress((spu_struct_p->uiAddr), uiSPUChannel);//20110719
	...
	//printk("phase = 0x%x\n",spu_struct_p->uiPhaseHigh);//20110715
	//gpHalSPU_SetPhase(0x0000aaab, uiSPUChannel);
	gpHalSPU_SetPhase(spu_struct_p->uiPhaseHigh, uiSPUChannel);
	gpHalSPU_SetPhaseAccumulator(0x0000, uiSPUChannel);
	gpHalSPU_SetTargetPhase(0x0000, uiSPUChannel);
	gpHalSPU_SetPhaseOffset(0x0000, uiSPUChannel);
	...
	//printk("SPU_PlayPCM_NoEnv_FixCH() exit\n");//20110705 test
}
//---------------------------------------------------------------------------------------------------------mixer
...
void load_tone_color_phys_addr(void)
{
	...
	for(i=0;i<spu_struct_p->total_inst;i++)
	{
		...
		T_InstrumentSectionPhyAddr[i] = out_ptr;
		//SPU_DBG("T_InstrumentSectionPhyAddr[%d] = 0x%x \n", i, out_ptr);
		T_InstrumentSectionKAddr[i] = (UINT32*)gp_chunk_va(out_ptr);
		//SPU_DBG("new T_InstrumentSectionKAddr[%d] = 0x%x \n", i, (UINT32)T_InstrumentSectionKAddr[i]);
	}	
	for(i=0;i<spu_struct_p->total_drum;i++)
	{
			...
			T_DrumPhyAddr[i] = out_ptr;
			//SPU_DBG("T_DrumPhyAddr[%d] = 0x%x \n",i, out_ptr);
			T_DrumKAddr[i] = (UINT32*)gp_chunk_va(out_ptr);
			//SPU_DBG("new T_DrumKAddr[%d] = 0x%x \n", i, (UINT32)T_DrumKAddr[i]);
	}
}
//-------------------------------------------------------------------------------------------------------------
/**
...
)
{
	...
	{
		filp->private_data = spu_devices;
		printk(KERN_WARNING "SPU has been opened\n");
	}
...
*/
#ifdef CONFIG_PM
	case SPU_SUSPEND://20110623
		spu_suspend1();
		break;	
		
	case SPU_RESUME://20110623
		spu_resume1();
		break;
#endif
	
	case SPU_STOP_CHANNEL://20110715
		copy_from_user((void*)&i, (const void __user *)arg, sizeof(int));
		SPU_StopChannel((unsigned int)i);
		break;
		
	case SPU_PAUSE_CHANNEL://20110715
		copy_from_user((void*)&i, (const void __user *)arg, sizeof(int));
		SPU_pause_channel((UINT8)i);
		break;
		
	case SPU_RESUME_CHANNEL://20110715
		copy_from_user((void*)&i, (const void __user *)arg, sizeof(int));
		SPU_resume_channel((UINT8)i);
		break;
	
	case SPU_GET_CHANNEL_STATUS://20110715
		copy_from_user((void*)&i, (const void __user *)arg, sizeof(int));
		ret = gpHalSPU_Get_SingleChannel_Status((UINT8)i);
		break;
	...
	case SPU_PLAY_DRM://play *.drm  20110503
			...
			temp = gp_user_va_to_pa((void*)drm_struct.pAddr);
			drm_struct.pAddr = (UINT32*)gp_chunk_va(temp);
			
			SPU_PlayPCM_NoEnv_FixCH((UINT32 *)drm_struct.pAddr, 
										drm_struct.phyAddr, 
										drm_struct.uiPan, 
										drm_struct.uiVelocity, 
										drm_struct.uiSPUChannel
										);
			break;	
	
	case SPU_SET_DRM_VOL://20110715
		copy_from_user((void *)&drm_struct, (const void __user *)arg, sizeof(STRUCT_DRM_SPU));
		gpHalSPU_SetVelocity((UINT8)drm_struct.uiVelocity, (UINT8)drm_struct.uiSPUChannel);
		break;			
		

	case SPU_MIDI_INITIAL://20110512
		MidiPlay_Initialize();
		break;
	...
	case SPU_GET_MIDI_STATUS:
		copy_from_user((void *)&spu_struct_temp, (const void __user *) arg, sizeof(STRUCT_MIDI_SPU));
		spu_struct_temp.MIDI_PlayFlag = spu_struct.MIDI_PlayFlag;
		copy_to_user ((void __user *)arg, (const void *)&spu_struct_temp, sizeof(STRUCT_MIDI_SPU));
		break;

	case SPU_MIDI_PLAY:
		copy_from_user((void *)&spu_struct, (const void __user *) arg, sizeof(STRUCT_MIDI_SPU));
		//spu_struct_p = (STRUCT_MIDI_SPU*)&spu_struct;//uncom by ggg 0913
		//SPU_DBG("*****************************************before addr mapping = 0x%x \n",(UINT32)spu_struct.midi_ring_buffer_addr);	
		temp = gp_user_va_to_pa((void*)spu_struct.midi_ring_buffer_addr);
		midi_buffer_Kaddr = (UINT32)gp_chunk_va(temp);
		//SPU_DBG("*****************************************after addr mapping = 0x%x \n",(UINT32)midi_buffer_Kaddr);	

		//SPU_DBG("*****************************************before addr mapping = 0x%x \n",(UINT32)spu_struct.pMIDI_DataPointer);	
		temp = gp_user_va_to_pa((void*)spu_struct.pMIDI_DataPointer);
		ggg_pMIDI_DataPointer = gp_chunk_va(temp);
		//SPU_DBG("*****************************************after addr mapping = 0x%x \n",(UINT32)ggg_pMIDI_DataPointer);	
	...
	case SPU_MIDI_STOP:
			...
			spu_struct.total_inst = 0;//20110513
			spu_struct.total_drum = 0;//20110513
			break;
	...
	case SPU_MIDI_SET_VOL:
			copy_from_user((void *)&spu_struct_temp, (const void __user *) arg, sizeof(STRUCT_MIDI_SPU));
			spu_struct.R_MIDI_Volume = spu_struct_temp.R_MIDI_Volume;
	...
	case SPU_MIDI_SET_PAN://20110412
			copy_from_user((void *)&spu_struct_temp, (const void __user *) arg, sizeof(STRUCT_MIDI_SPU));
			spu_struct.R_MIDI_Pan = spu_struct_temp.R_MIDI_Pan;
			SPU_MIDI_Set_MIDI_Pan();
			break;
	case SPU_MIDI_SET_SPU_Channel_Mask://20110512
			copy_from_user((void *)&spu_struct_temp, (const void __user *) arg, sizeof(STRUCT_MIDI_SPU));
			spu_struct.R_MIDI_CH_MASK = spu_struct_temp.R_MIDI_CH_MASK;
			spu_struct.R_SourceMIDIMask = spu_struct_temp.R_SourceMIDIMask;
			break;
	case SPU_MIDI_BUF_WR:
			copy_from_user((void *)&spu_struct_temp, (const void __user *) arg, sizeof(STRUCT_MIDI_SPU));
			spu_struct.midi_ring_buffer_wi = spu_struct_temp.midi_ring_buffer_wi;
			spu_struct.remain_midi_length = spu_struct_temp.remain_midi_length;
			spu_struct.static_midi_ring_buffer_ri = spu_struct_temp.static_midi_ring_buffer_ri;
			spu_struct.currt_midi_data_offset = spu_struct_temp.currt_midi_data_offset;
			break;

	case SPU_MIDI_BUF_RD:
			copy_from_user((void *)&spu_struct_temp, (const void __user *) arg, sizeof(STRUCT_MIDI_SPU));//20110511
			spu_struct.midi_ring_buffer_addr = spu_struct_temp.midi_ring_buffer_addr;
			spu_struct.midi_ring_buffer_wi = spu_struct_temp.midi_ring_buffer_wi;
			spu_struct.remain_midi_length = spu_struct_temp.remain_midi_length;//20110511
			spu_struct.static_midi_ring_buffer_ri = spu_struct_temp.static_midi_ring_buffer_ri;//20110511
			spu_struct.currt_midi_data_offset = spu_struct_temp.currt_midi_data_offset;
/*			
			SPU_DBG("spu_struct.midi_ring_buffer_addr = 0x%x \n",spu_struct.midi_ring_buffer_addr);
			SPU_DBG("spu_struct.midi_ring_buffer_wi = 0x%x \n",spu_struct.midi_ring_buffer_wi);
			SPU_DBG("spu_struct.remain_midi_length = 0x%x \n",spu_struct.remain_midi_length);
			SPU_DBG("spu_struct.static_midi_ring_buffer_ri = 0x%x \n",spu_struct.static_midi_ring_buffer_ri);
			SPU_DBG("spu_struct.currt_midi_data_offset = 0x%x \n",spu_struct.currt_midi_data_offset);
*/			
...
#ifdef CONFIG_PM
	platform_device_unregister(&spu_device);
	platform_driver_unregister(&spu_driver);//20110623
#endif
}
...
)
{
	...
	//return 0;//20110623 mask
	#ifdef CONFIG_PM
	platform_device_register(&spu_device);
	platform_driver_register(&spu_driver);//20110623
	#endif
	return 0;//20110628
