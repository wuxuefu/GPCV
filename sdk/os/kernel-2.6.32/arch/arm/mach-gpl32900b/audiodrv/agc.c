...
{
	agc_para.PGA_low_bound = para->PGA_low_bound;
	agc_para.PGA_upper_bound = para->PGA_upper_bound;
	agc_para.level_low_bound = para->level_low_bound;
	agc_para.level_upper_bound = para->level_upper_bound;
}
...
//	sar_audio_SET_MIC_boost(false); // Enable/disable 20dB boost        
	agc_para.PGA_low_bound = 1;
	agc_para.PGA_upper_bound = 31;
	agc_para.level_low_bound = 1500;
	agc_para.level_upper_bound = 6500;
...
//		y = *pBuffPtrRead++;
	...
	if( (max_level) > agc_para.level_upper_bound ) {
		I2SRx_Mic_VolGet(&x);
		x--;//=3;
		if(x < agc_para.PGA_low_bound) x = agc_para.PGA_low_bound; // lower bound of PGA
		I2SRx_Mic_VolSet(x);
	//	printk("+++++++++++++++++++++++++++++++++++++++%d\n", x);
	} else {
		if( max_level < agc_para.level_low_bound ) { 
            I2SRx_Mic_VolGet(&x); 
            x++; 
            if(x > agc_para.PGA_upper_bound) x = agc_para.PGA_upper_bound; // upper bound of PGA 
            I2SRx_Mic_VolSet(x);
     //       printk("----------------------------------------%d\n",x);
        } 
