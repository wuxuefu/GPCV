...
#define	HDMII2S_BLK_SIZE	(16*1024)
#define HDMII2S_BLK_NUM		(4)
...
/**
 * @breif Audio select output.
 * @param filp The file structure pointer.
 * @param num  Tx:I2S, HDMI  Rx:I2S
 */
static int dsp_ioctl_gp_init(struct file *filp, int num)
{
	struct dsp_dev_s *dev = (struct dsp_dev_s *)filp->private_data;
	...
	if ( ((filp->f_flags & O_ACCMODE) == O_WRONLY) || ((filp->f_flags & O_ACCMODE) == O_RDWR) )
	{
		...
		if ( num == HDMI_TX )
		{
			...
			buf_status_destroy(&dma->playBuf);
			dev->is_tx_start = 0;
			if (buf_status_init(pplayBuf, HDMII2S_BLK_SIZE, HDMII2S_BLK_NUM) != 0) {
				DIAG_ERROR("DSP device no memory ! (play)\n");
				return -ENOMEM;
			}

			/* enable i2s_hdmi */
			audio_i2s_hdmi_init(1, dev->chans_tx, dev->bits, dev->hw.hw_freq);
		} else {
			audio_i2shdmi_disable();
			buf_status_destroy(&dma->playBuf);
			dev->is_tx_start = 0;
 ...
 */
static int dsp_ioctl_sync(struct file *filp, int timeout)
{
	dsp_dev_t *dev = filp->private_data;
	dma_info_t *dma = &dev->dma;
	buf_status_t *pplayBuf = &dma->playBuf;
...
//		return 0;
	...
	if (flags == O_WRONLY) {
		...
		if(dev->is_tx_start == 0) {
			if( audio_block_queue_get_count(&pplayBuf->q_ready) == 1 ) {
				pplayBuf->a_addr = audio_block_queue_get(&pplayBuf->q_ready);
				pplayBuf->b_addr = pplayBuf->dummy_addr;
			} else if( audio_block_queue_get_count(&pplayBuf->q_ready) == 2 ) {
				pplayBuf->a_addr = audio_block_queue_get(&pplayBuf->q_ready);
				pplayBuf->b_addr = audio_block_queue_get(&pplayBuf->q_ready);
			} else if( audio_block_queue_get_count(&pplayBuf->q_ready) == 0 ) {
				return 0;
			} else {
				printk("Error: tx_start=0, ready buffer>2 (%d)\n", audio_block_queue_get_count(&pplayBuf->q_ready));
			}
			if(dev->type == I2S_TX) {
				...
				audio_ocpdma_setbuf(DMA_BUFFER_A, (char *)virt_to_phys((void *)pplayBuf->a_addr), pplayBuf->q_ready.blk_size);
				audio_ocpdma_setbuf(DMA_BUFFER_B, (char *)virt_to_phys((void *)pplayBuf->b_addr), pplayBuf->q_ready.blk_size);
				audio_ocpdma_en();
			}
			dev->is_tx_start = 1;	
		}
...
//				(dma->playBuf.b_addr == dma->playBuf.dummy_addr)) {
			if ((dma->playBuf.a_addr == dma->playBuf.dummy_addr) &&
				(dma->playBuf.b_addr == dma->playBuf.dummy_addr)) {
				/* stop the hardware */
			/*	if(dev->type!=HDMI_TX) {
					audio_i2stx_disable();
...
static irqreturn_t ocpdma_irq_handle_play(int irq, void *dev_id)
{
	struct dsp_dev_s *dev;
	struct dma_info_s *dma;
	...
	if (dmaplaybuf==DMA_BUFFER_A) {
		/* busy on buffer B*/
		/* setup the address of A */
		audio_ocpdma_setbuf(DMA_BUFFER_A, (char *)virt_to_phys((void *)addr), pplayBuf->q_ready.blk_size);
	...
	} else if(dmaplaybuf==DMA_BUFFER_B) {
		/* busy on buffer A*/
		/* setup the address of B */
		audio_ocpdma_setbuf(DMA_BUFFER_B, (char *)virt_to_phys((void *)addr), pplayBuf->q_ready.blk_size);
			      ...
			      struct file *filp)
{
	...
	/* play back */
	audio_freq_set(44100, 1);
	dev->hw.hw_freq = 44100;
	dev->type = 0;
	dev->chans_tx = 0;
	dev->bits = 0;
	...
	result = request_irq(IRQ_I2STX1, ocpdma_irq_handle_play, IRQF_DISABLED, "I2S_TX_DMA", dev);
	if (result) {
		dsp_dev_release_write(inode, filp);
		DIAG_ERROR("Audio: Request IRQ (play) failed\n");
		return -ENODEV;
		  ...
		  unsigned long arg)
{
	...
	switch (cmd) {
		...
		case SNDCTL_DSP_CHANNELS:
			...
			break;
		case SNDCTL_DSP_GP_INIT:
            ret = dsp_ioctl_gp_init(filp, *(int __user*)arg);
            break;
		...
		case SNDCTL_DSP_SYNC:	/* Suspend the application until all samples have been played */
			timeout = DEFAULT_BLK_SIZE * DEFAULT_BLK_NUM * 100 / dev->hw.hw_freq;
			ret = dsp_ioctl_sync(filp, timeout);
			break;
		...
		case SNDCTL_DSP_POWEROFF:
			audio_power_ctrl(0);
			break;
		case SNDCTL_DSP_SET_AGC:
			agc_para_setting((audio_agc_setting *)arg);
			break;
		default:
			printk("IOCTL:%d\n", cmd);
	}
