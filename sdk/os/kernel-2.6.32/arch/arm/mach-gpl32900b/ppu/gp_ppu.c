...
)
{
  ...
  #elif PPU_GPIO_SET == 2
		pin = (0<<24)|(2<<16)|(31<<8)|29;//EVBV1.1->JP22->TX PIN(UART0)
...
*/
signed int 
gp_ppu_text_bitmap_flip_enable_set(
PPU_REGISTER_SETS *p_register_set, 
unsigned int value
)
{
...
EXPORT_SYMBOL(gp_ppu_text_bitmap_flip_enable_set);
...
EXPORT_SYMBOL(gp_ppu_tv_flip_enable_set);
...
EXPORT_SYMBOL(gp_ppu_tft_flip_enable_set);
...
EXPORT_SYMBOL(gp_ppu_argb888_enable_set);
...
)
{
        ...
        #endif
     #else
        if ((gpHalPPUGetIrqStatus() & 0x1) == 0) {
		         return IRQ_NONE;
	      }
	      gpHalPPUClearIrqFlag(0x1);
        gpHalPPUIsr();
     #endif           
...
EXPORT_SYMBOL(gp_ppu_fblock_set);
...
EXPORT_SYMBOL(gp_ppu_irq_state_get);
/**
...
EXPORT_SYMBOL(gp_ppu_enable_set);
/**
...
)
{
  ...
  /* initial ppu register parameter set structure */
  	...
  	copy_from_user((void*)ppu_register_set, (void __user*)arg, sizeof(PPU_REGISTER_SETS));  
  #endif
  ...
  switch(cmd)
  {
      ...
      case PPUIO_SET_TEXT_BITMAP_FLIP_ENABLE:              
           ret=gp_ppu_text_bitmap_flip_enable_set(ppu_register_set, (unsigned int)ppu_register_set->ppu_set.PPU_enable_mode);
           break;

      case PPUIO_SET_TV_FLIP_ENABLE:              
           ret=gp_ppu_tv_flip_enable_set(ppu_register_set, (unsigned int)ppu_register_set->ppu_set.PPU_enable_mode);
           break;
           
      case PPUIO_SET_TFT_FLIP_ENABLE:              
           ret=gp_ppu_tft_flip_enable_set(ppu_register_set, (unsigned int)ppu_register_set->ppu_set.PPU_enable_mode);
           break;
      ...
      case PPUIO_SPRITE_SET_ATTRIBUTE_CHARNUM:               
           ...
           (unsigned int)ppu_register_set->ppu_set.PPU_hsize_mode);
           break;  
...
{
#if 0
	dev_t devno = MKDEV(ppu_devices->ppu_major, PPU_MINOR);
	
	device_destroy(ppu_devices->ppu_class, devno);
	cdev_del(&ppu_devices->c_dev);
	free_irq(VIC_PPU, ppu_devices);	
	class_destroy(ppu_devices->ppu_class);
	kfree(ppu_devices);
	ppu_devices = NULL;
...
)
{
  ...
  DEBUG(KERN_WARNING "PPU module init\n");
  
  /* allocate a major number to ppu module */	
	result = alloc_chrdev_region(&dev, PPU_MINOR, 1, "PPU");
	if(result < 0){
		 DEBUG("PPU: can't get major\n");
		 goto fail_init;
	}
	
	/* allocate a structure for ppu device */
	ppu_devices = kmalloc(sizeof(gp_ppu_dev_t), GFP_KERNEL);
	if(!ppu_devices) {
		 DEBUG("PPU: can't kmalloc\n");
		 result = -ENOMEM;
		 goto fail_kmalloc;
	}
	memset(ppu_devices, 0, sizeof(gp_ppu_dev_t));		
	ppu_devices->ppu_major = MAJOR(dev);
	
	/* create class for csi character device */
  ppu_devices->ppu_class = class_create(THIS_MODULE, "PPU");
	if (IS_ERR(ppu_devices->ppu_class))
	{
		 DEBUG("PPU: can't create class\n");
		 result = -EFAULT;
		 goto fail_create_class;
	}	
	
	/* request a irq forppu interrupt service */
	result = request_irq(VIC_PPU,gp_ppu_irq_handler,IRQF_SHARED,"PPU_IRQ",&ppu_devices);	
	if(result < 0) {
		 DEBUG("PPU: request ppu irq fail\n");
		 goto fail_request_irq;
	}		
  
  /* create character device node */
	devno = MKDEV(ppu_devices->ppu_major, PPU_MINOR);
	...
	if(result < 0)
	{
		DEBUG("PPU: cdev_add error\n");
		goto fail_device_register;
	}

	device = device_create(ppu_devices->ppu_class, NULL, devno, NULL, "ppu%d", 0);
	if(!device){
		DEBUG("PPU: device_create error\n");
  ...
  platform_device_register(&ppu_module_device);
	return platform_driver_register(&ppu_module_driver);	
...
fail_init:
  DEBUG(KERN_WARNING "PPU module init failed \n");
	
	return result;
#else
	int nRet;

	DEBUG(KERN_WARNING "ModuleInit: PPU \n");
