...
SINT32 gpHalScuClkEnable(UINT32 bitMask, UINT8 scu, UINT8 enable)
{
	...
	static int apbdma_cnt=0, saadc_cnt=0, pputv1_cnt=0, clkppu_cnt=0, clkppureg_cnt=0, clkppufb_cnt=0;
	...
	case SCU_A :
	{
		scuaReg_t *pScuaReg = (scuaReg_t *)LOGI_ADDR_SCU_A_REG;
		if (enable){
			if(bitMask & SCU_A_PERI_APBDMA_A)
				apbdma_cnt++;
			if(bitMask & SCU_A_PERI_SAACC)
				saadc_cnt++;
			...
			if(bitMask & SCU_A_PERI_APBDMA_A) {
				apbdma_cnt--;
				if(apbdma_cnt>0)
					bitMask = bitMask & ~SCU_A_PERI_APBDMA_A;
				else
					apbdma_cnt=0;
			}
			if(bitMask & SCU_A_PERI_SAACC) {
				saadc_cnt--;
				if(saadc_cnt>0)
					bitMask = bitMask & ~SCU_A_PERI_SAACC;
				else
					saadc_cnt=0;
			}			
			pScuaReg->scuaPeriClkEn &= ~bitMask;
	...
	case SCU_A2:
	{
		scuaReg_t *pScuaReg = (scuaReg_t *)LOGI_ADDR_SCU_A_REG;
		if (enable){
			if(bitMask & SCU_A_PERI_PPU)
				clkppu_cnt++;
			if(bitMask & SCU_A_PERI_PPU_REG)
				clkppureg_cnt++;
			if(bitMask & SCU_A_PERI_PPU_FB)
				clkppufb_cnt++;									
			...
			if(bitMask & SCU_A_PERI_PPU) {
				clkppu_cnt--;
				if(clkppu_cnt>0)
					bitMask = bitMask & ~SCU_A_PERI_PPU;
				else
					clkppu_cnt=0;
			}	
			if(bitMask & SCU_A_PERI_PPU_REG) {
				clkppureg_cnt--;
				if(clkppureg_cnt>0)
					bitMask = bitMask & ~SCU_A_PERI_PPU_REG;
				else
					clkppureg_cnt=0;
			}					
			if(bitMask & SCU_A_PERI_PPU_FB) {
				clkppufb_cnt--;
				if(clkppufb_cnt>0)
					bitMask = bitMask & ~SCU_A_PERI_PPU_FB;
				else
					clkppufb_cnt=0;
			}					
			pScuaReg->scuaPeriClkEn2 &= ~bitMask;
	...
	case SCU_A3:
	{
		scuaReg_t *pScuaReg = (scuaReg_t *)LOGI_ADDR_SCU_A_REG;
		if (enable){
			if(bitMask & SCU_A_PERI_TV1CLK27_EN)
				pputv1_cnt++;			
			pScuaReg->scuaSysSel |= bitMask;
		}
		else{
			if(bitMask & SCU_A_PERI_TV1CLK27_EN) {
				pputv1_cnt--;
				if(pputv1_cnt>0)
					bitMask = bitMask & ~SCU_A_PERI_TV1CLK27_EN;
				else
					pputv1_cnt=0;
			}			
			pScuaReg->scuaSysSel &= ~bitMask;
	...
	case SCU_B_PZONEE :
	{
		scubReg_t *pScubReg = (scubReg_t *)LOGI_ADDR_SCU_B_REG;
		if (enable){
			pScubReg->scubPZoneEClkEn |= bitMask;
		}
		else{
			pScubReg->scubPZoneEClkEn &= ~bitMask;
		}
...
void gpHalScuUsbPhyClkEnable ( int enable )
{
    ...
    if( enable ) {
        pScuaReg->scuaUsbPhyCfg1 = pScuaReg->scuaUsbPhyCfg1 | 0x04;
    }
    else{
        pScuaReg->scuaUsbPhyCfg1 = pScuaReg->scuaUsbPhyCfg1 & 0xfffffffb;
    }
}
...
SINT32 gpHalClkSpll2SetRate(UINT32 rate, UINT32 *realRate)
{
  ...
  if(rate > 800000000)
	...
	pscubReg->scubSpll2Cfg &= ~0xff;
	pscubReg->scubSpll2Cfg |= ratio;
