...
/**
 * @brief timer/counter LDR load en regsiter set
 * @param id [in] timer/counter index
 * @param mode [in] 0:disable load; 1:enable load 
 */
void gpHalTimerLDENSet(int id, int en)
{
	int temp;	
	timerReg_t *ptimerReg = (timerReg_t *)(LOGI_ADDR_TIMER_REG + id*LOGI_TIMER_OFFSET);
	...
	if(en){
		ptimerReg->tmCtr = temp|TMR_LD_EN;
	}
	else{
		ptimerReg->tmCtr = temp&(~TMR_LD_EN);
	}
