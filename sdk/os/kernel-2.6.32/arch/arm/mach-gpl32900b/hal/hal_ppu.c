	   ...
	   if(fb_lock->fb_lock_en)
	   {
			   ...
			   switch(fb_lock->TV_color_mode)
			   {
				      ...
				      default:
				           ...
				           gpHalPPUSetFbMono(fb_lock->TV_color_mode);
				           break;				      		             			           			           
			   }
			   gpHalPPUSetVgaEnable(0);
			   gpHalPPUSetRes(0);
			   pPPUFunreg->ppuFreesize = (((0 << B_FREE_INIT) & MASK_FREE_INIT_SIZE)|((fb_lock->TV_frame_hsize_mode << B_FREE_H_SIZE) & MASK_FREE_H_SIZE )|(fb_lock->TV_frame_vsize_mode & MASK_FREE_V_SIZE));
			   pPPUTVFunreg->ppuTvEn = 0x1;
			   #if 0
			   pPPUTVFunreg->ppuTvEn = temp;
			   #endif
			   
			   // TFT
			   temp = pPPUTFTFunreg->ppuTftEn;
			   ...
			   gpHalPPUSetVgaEnable(0);
			   gpHalPPUSetRes(0);
			   pPPUFunreg->ppuFreesize = (((0 << B_FREE_INIT) & MASK_FREE_INIT_SIZE)|((fb_lock->TFT_frame_hsize_mode << B_FREE_H_SIZE) & MASK_FREE_H_SIZE )|(fb_lock->TFT_frame_vsize_mode & MASK_FREE_V_SIZE));			   
			   pPPUTFTFunreg->ppuTftEn = 0x1; 
			   #if 0
			   pPPUTFTFunreg->ppuTftEn = temp;
			   #endif                  
	   }
	   else
       gpHalPPUFbCtrl(0);
