...
{
	//int i;
	gp_enable_clock( (int*)"SPU", 1 );
	gpHalSPU_RegWrite((unsigned int)&(pScuaReg->scuaSysSel), 0x02000080);

}	

void gpHalSpuClkDisable(
	void
)
{
	gp_enable_clock( (int*)"SPU", 0 );
}
...
void gpHalSPU_LpfEnable(void)
{
	gpHalSPU_RegModifyWrite((unsigned int)&(pSpuReg->spuPostWavCtrl), 0xFFFFFFFF, C_LPF_ENABLE);
}
void gpHalSPU_LpfDisable(void)
{
	gpHalSPU_RegModifyWrite((unsigned int)&(pSpuReg->spuPostWavCtrl), ~C_LPF_ENABLE, 0);	
}
/**
