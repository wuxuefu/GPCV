...
)
{
	...
	if(enable)
			pdisp2Reg->disp2Ctrl2 |= 0x1;
  else
  	  pdisp2Reg->disp2Ctrl2 &= ~0x1;
}
EXPORT_SYMBOL(gpHalDisp2SetD1Position);

void
gpHalDisp2Set576Mode(
	UINT32 enable
)
{
	disp2Reg_t *pdisp2Reg = (disp2Reg_t *)(LOGI_ADDR_DISP2_REG);
  ...
  if(enable)
		pdisp2Reg->disp2Ctrl2 |= 0x2;
  else
    pdisp2Reg->disp2Ctrl2 &= ~0x2;  
}
EXPORT_SYMBOL(gpHalDisp2Set576Mode);

void
gpHalDisp2Set480IMode(
	UINT32 enable
)
{
	disp2Reg_t *pdisp2Reg = (disp2Reg_t *)(LOGI_ADDR_DISP2_REG);
	gpSysReg_t *pTVsysreg = (gpSysReg_t *)(SYSTEM_BASE_REG);
  
  if(enable)
  {
		pdisp2Reg->disp2Ctrl2 |= 0x4;
...
EXPORT_SYMBOL(gpHalDisp2Set480IMode);

void
gpHalDisp2Set720PMode(
	UINT32 enable
)
{
	disp2Reg_t *pdisp2Reg = (disp2Reg_t *)(LOGI_ADDR_DISP2_REG);
  ...
  else
  {
    ...
    pTVsysreg->gpTvEnable1 |=~0x18;
    pTVsysreg->gpTvEnable2 &=~0xC0000; 
    pdisp2Reg->disp2Ccmoffse = 0x0;       
  }
}
EXPORT_SYMBOL(gpHalDisp2Set720PMode);

signed int gpHalDisp2GetypbprMode(void)
{	
	 gpSysReg_t *pTVsysreg = (gpSysReg_t *)(SYSTEM_BASE_REG);
	 
	 return (pTVsysreg->gpTvEnable0 & 0x14);
}
EXPORT_SYMBOL(gpHalDisp2GetypbprMode);
