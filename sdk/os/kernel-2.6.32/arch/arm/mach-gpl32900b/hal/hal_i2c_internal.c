...
#include <mach/hal/regmap/reg_i2c_internal.h>
...
)
{
	...
	i2cBusReg->idsr = (slaveAddr & 0xff) | 0x01;

	iccr |= ICCR_ACKEN;
	ctrl = ICSR_START | ICSR_MASTER_RX | ICSR_TXRX_ENABLE;

	i2cBusReg->iccr = iccr;
	i2cBusReg->icsr = ctrl;

	ret = (HAL_BUSY_WAITING(i2cBusReg->iccr & ICCR_INTPREND, 60) >= 0) ? SP_OK : SP_FAIL;
