...
/**
* @brief	aes reorder set
* @param 	type[in]: 0 is encrypt; 1 is decrypt.
* @param 	field[in]: 0 is key; 1 is text in; 2 is test out.
* @param 	value[in]: re-order control value.
* @return 	none
*/
void
gpHalAesSetReorder(
...
{
	static unsigned int reorderValue = 0;
	UINT32 shift = 0;

	if (type == AES_REORDER_NONE) {
		reorderValue = 0;
		aesReg->aesReorder = 0;
		return;
	}
	else if (type == AES_REORDER_DECRYPT) {
		shift = 16;
	}

	switch (field) {
		case AES_REORDER_KEY:		shift += 0;		break;
		case AES_REORDER_TEXT_IN:	shift += 5;		break;
		case AES_REORDER_TEXT_OUT:	shift += 10;	break;
	}

	reorderValue &= ~(0x1F << shift);;
	reorderValue |= value << shift;
	aesReg->aesReorder = reorderValue;
}
