...
static void disp2_panel_suspend(void);
static void disp2_panel_resume(void);
...
)
{
	...
	/* Is panel available? */
	if (gpDisp2Info->outputDev.ops == NULL ||
		gpDisp2Info->outputDev.ops->init == NULL) {
	...
	ret = (gpDisp2Info->outputDev.ops->init)();
	if (ret < 0) {
		return ret;
	}

	/* Set data path to TFT1 or TV1 */
	if (gpDisp2Info->outputDev.type == SP_DISP_OUTPUT_TV) {
		#if 0
			SCUA_SAR_GPIO_CTRL |= 0x21;
		#else
			gpHalScuClkEnable( SCU_A_PERI_PPUTV1_EN | SCU_A_PERI_TV1CLK27_EN , SCU_A3, 1);
	  #endif
	}
	else {
		#if 0
		...
		#else
			gpHalScuClkEnable( SCU_A_PERI_PPUTV1_EN | SCU_A_PERI_TV1CLK27_EN , SCU_A3, 0);
			gpHalScuClkEnable( SCU_A_PERI_PPUTFT1_EN , SCU_A3, 1);
	  #endif
	}
...
)
{
	...
	gpDisp2Info->outputEnable = 1;
	return disp2_switch_panel();
}
...
{
	if (gpDisp2Info->outputDev.ops->suspend)
		(gpDisp2Info->outputDev.ops->suspend)();
...
{
	if (gpDisp2Info->outputDev.ops->resume)
		(gpDisp2Info->outputDev.ops->resume)();
...
)
{
	...
	if (panelType != SP_DISP_OUTPUT_TV) {
		printk("[%s:%d], error, disp2 only support TV\n", __FUNCTION__, __LINE__);
		return -1;
	}

	gpDisp2Info->outputDev.type = panelType;

	memset(gpDisp2Info->outputDev.name, 0, sizeof(gpDisp2Info->outputDev.name));
	strncpy(gpDisp2Info->outputDev.name, name, strlen(name));

	gpDisp2Info->outputDev.ops = panelOps;
...
)
{
	...
	gpDisp2Info->outputDev.ops = NULL;
...
)
{
	...
	if (gpDisp2Info->updateFlag) {
       ...
       if(!gpHalPPUGetFbEnable())
       {
          gpHalPPUSetFbEnable(1);
					if(gpDisp2Info->panelSize.width == 320)
					  gpHalDisp2SetRes(HAL_DISP2_RES_320_240);
					else if(gpDisp2Info->panelSize.width == 640)
					  gpHalDisp2SetRes(HAL_DISP2_RES_640_480);
					else
					  gpHalDisp2SetRes(HAL_DISP2_RES_720_480);            
       }     
    }
...
)
{
	...
	if(device_open_count == 0)
	{
			...
			SCUA_SAR_GPIO_CTRL |= 0x21;
		#else
			gpHalScuClkEnable( SCU_A_VDEC_POWER_DOWN , SCU_A4, 0);
			gpHalScuClkEnable( SCU_A_VDEC_74M_EN , SCU_A4, 0);
			gpHalScuClkEnable( SCU_A_VDEC_REF_EN , SCU_A4, 1);
			gpHalScuClkEnable( SCU_A_VDEC_YPBPR_EN , SCU_A4, 1);
			//dac_en(1);
			gpHalScuClkEnable( SCU_A_PERI_PPUTV1_EN | SCU_A_PERI_TV1CLK27_EN , SCU_A3, 1);
		#endif
	}
...
)
{
	...
	{
		#if 0
			SCUA_SAR_GPIO_CTRL &= ~0x31;
		#else
			gpHalScuClkEnable( SCU_A_PERI_PPUTV1_EN | SCU_A_PERI_TV1CLK27_EN |SCU_A_PERI_PPUYPBPR_EN | SCU_A_PERI_TV1CLK74_EN, SCU_A3, 0);
			gpHalScuClkEnable( SCU_A_VDEC_74M_EN , SCU_A4, 0);
			gpHalScuClkEnable( SCU_A_VDEC_REF_EN , SCU_A4, 0);
			gpHalScuClkEnable( SCU_A_VDEC_YPBPR_EN , SCU_A4, 1);
			gpHalScuClkEnable( SCU_A_VDEC_POWER_DOWN , SCU_A4, 1);
		#endif
  }
	/* Success */
	return 0;
...
)
{
	...
	switch (cmd) {
		...
		case DISPIO_SET_TV_MODE:
				if (gpDisp2Info->outputDev.ops->set_param) {
					if ((gpDisp2Info->outputDev.ops->set_param)(&arg) < 0) {
						err = -EIO;
						break;
					}
				}
...
)
{
  ...
  gpDisp2Info->tvMode = SP_DISP_TV_MODE_NTSC;
	...
	/* irq request */
	retval = request_irq(IRQ_PPU, disp2_irq, IRQF_SHARED, "PPU_IRQ", (void *) gpDisp2Info);
	if (retval) {
...
fail_register:
	free_irq(IRQ_PPU, (void *)gpDisp2Info);
fail_irq:
	kfree(gpDisp2Info);
	gpDisp2Info = NULL;
fail_malloc:
