...
static unsigned int mmu_0mb = 0, mmu_1mb = 0, mmu_b00mb = 0;
...
static struct proc_dir_entry *proc_speedy_devtype;
...
static int read_proc_speedy_devtype(char *page, char **start, off_t offset,
                                 int count, int *eof, void *data)
{
    int len;

    len = sprintf(page, "%d\n", speedy_param.snapshot_dev);
    *eof = 1;
    return len;
}

static int write_proc_speedy_devtype(struct file *file, const char *buffer,
                                  unsigned long count, void *data)
{
    int val;

    if ((val = write_proc_speedy(buffer, count, 3, "devtype")) < 0)
        return val;
    speedy_param.snapshot_dev = val;
    return count;
}
...
static int __init speedy_init(void)
{
    ...
    if ((proc_speedy = proc_mkdir("speedy", NULL))) {
        ...
        if ((proc_speedy_devtype =
             create_proc_entry("devtype",
                               S_IRUGO | S_IWUSR,
                               proc_speedy))) {
            proc_speedy_devtype->read_proc = read_proc_speedy_devtype;
            proc_speedy_devtype->write_proc = write_proc_speedy_devtype;
        }
    }
#endif
