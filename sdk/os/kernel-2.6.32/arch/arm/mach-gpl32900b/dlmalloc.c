...
)
{
	...
	if (max_Csize > (CHUNK_OVERHEAD + CHUNK_ALIGN_MASK)) {
		max_Csize = (max_Csize - CHUNK_OVERHEAD - CHUNK_ALIGN_MASK) & ~CHUNK_ALIGN_MASK;
	}
...
)
{
	...
	//diag_printf("[%s][%d] pgdl_GM(%08X)=%08X\n", __FUNCTION__, __LINE__, (UINT32)&pgdl_GM, (UINT32)pgdl_GM);
...
UINT32 dlGetMemBlock(void **addr)
{
	ptrMstate m = pgdl_GM;

	if (check_malloc_state(m) == SUCCESS) {
		if (is_initialized(m)) {
			void *mem;
			UINT32 size;
			mchunkptr pChunk;
			...
			while (s != 0){
				...
				while (segment_holds(s, pChunk) &&
					...
					if (is_inuse(pChunk)){
						...
						size = chunksize(pChunk) - PAGE_SIZE;

						if (*addr >= mem && *addr < mem + size) {
							*addr = mem;
							diag_printf("[dlGetMemBlock] addr = %08X\n", (UINT32)mem);
							diag_printf("[dlGetMemBlock] size = %08X\n", size);
							return size;
						}
					}
