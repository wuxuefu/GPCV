...
{
	gp_enable_clock( (int*)"TIMER", enable );
}
...
/**
 * @brief timer LDR load en  set
 * @param handle [in] timer/counter handle
 * @param mode [in] 1:enable load; 0:disable load 
 *  			 
 * @return success: 0,  erro: ERROR_ID
 */
int gp_tc_enable_ld(int handle, int en)
{
	struct gp_tc_s *timer = NULL;
	...
	gpHalTimerLDENSet(timer->id, en);
	
	return 0;
}
EXPORT_SYMBOL(gp_tc_enable_ld);
 ...
 * @return success: 0,  erro: ERROR_ID
 */
int gp_tc_set_load(int handle, unsigned int value)
{
	struct gp_tc_s *timer = NULL;
 ...
 * @return success: 0,  erro: ERROR_ID
 */
int gp_tc_get_load(int handle, unsigned int *value)
{
	struct gp_tc_s *timer = NULL;
 ...
 * @return success: 0,  erro: ERROR_ID
 */
int gp_tc_set_compare(int handle, unsigned int value)
{
	struct gp_tc_s *timer = NULL;
 ...
 * @return success: 0,  erro: ERROR_ID
 */
int gp_tc_get_compare(int handle, unsigned int *value)
{
	struct gp_tc_s *timer = NULL;
...
void gp_tc_set_freq_duty(int handle, UINT32 freq, UINT32 duty)
{
	...
	printk("temp == 0x%x\r\n",temp);
	printk("ld == 0x%x\r\n",0xffffffff -temp);
	gp_tc_set_load(handle,0xffffffff - temp); /* up counting */
	
	if((0 < duty)&&(duty < 100)){
		#if 0
		temp = (100 - duty)*temp/100 - 1;
		#else
		temp = (100 - duty)*temp/100 + 0xffffffff - temp;
		#endif
	}
	else if(0 == duty){ /* when cmp < ld pwm output low (unaccountability)*/
		temp--;  
	}
	else{ /* duty = 100 */
		temp = 0xffffffff - temp + 2;
	}
	 printk("compare == 0x%x\r\n",temp);
...
static int gp_tc_probe(struct platform_device *pdev)
{
	...
	if(!name)
		goto err_alloc;
	snprintf(name, 7, "timer%d", pdev->id);
