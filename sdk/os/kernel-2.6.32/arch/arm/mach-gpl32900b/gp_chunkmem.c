 ...
 */
void gp_chunk_info(unsigned int *base, unsigned int *size)
{
	*base = (unsigned int)chunkmem->vbase;
	*size = chunkmem->size;
}
 ...
 * @see 	dlMalloc
 */
void* gp_chunk_malloc(unsigned int size)
{
	return gp_chunk_malloc_ex(size, current->tgid);
}
 ...
 * @see 	dlMalloc
 */
void* gp_chunk_malloc_ex(unsigned int size, unsigned int id)
{
	void *ret = NULL;
	...
	if ((chunkmem != NULL) && (size != 0)) {
...
#else
		{
			...
			#else
			if (ret == NULL) {
				DIAG_ERROR("[gp_chunk_malloc_ex] out of memory! (%s:%08X)\n", current->comm, size);
				dlMalloc_Status(NULL);
			}
			#endif
			up(&chunkmem->sem);
...
EXPORT_SYMBOL(gp_chunk_malloc_ex);
...
static long chunkmem_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case CHUNK_MEM_MMAP:
		{
			...
			if (cmd == CHUNK_MEM_MMAP) {
				...
				{
					void *addr = ka;
					unsigned int size = dlGetMemBlock(&addr);
					if ((size == 0) || (ka + block.size > addr + size) ) {
						DIAG_ERROR("CHUNK_MEM_MMAP: invalid address or size! (%s:%08X:%08X)\n", current->comm, block.phy_addr, block.size);
						ret = -EFAULT;
						break;
					}
				}
			...
			va = do_mmap_pgoff(
				...
				MAP_SHARED,
				(chunkmem->pbase + (ka - chunkmem->vbase)) >> PAGE_SHIFT);
	...
	case CHUNK_MEM_INFO:
		{
			...
			if (info.pid == (unsigned int)(-2)) {
				info.used_bytes = *(unsigned int *)(chunkmem->vbase + 0xFD0);
			}
#endif
	...
	case CHUNK_MEM_FREE_ALL:
		gp_chunk_free_all(arg);
		break;
