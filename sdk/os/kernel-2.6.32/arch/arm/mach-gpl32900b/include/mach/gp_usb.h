...
#define AUDIO_SUPPORT		0	/* Support AUDIO device */
...
enum {
	USB_DEVICE_DETECTION_TYPE = 0,
	USB_DEVICE_MSDC_TYPE,
    USB_DEVICE_UVC_TYPE,
    USB_DEVICE_NONE_TYPE
};

enum {
	USB_DEVICE_CONNECT_TO_HOST = 0,
	USB_DEVICE_CONNECT_TO_CHARGER,
    USB_DEVICE_CONNECT_TO_NONE
};

/* usb uvc ioctl function, Eugene added */
enum
{
	USB_UVC_IDLE_STATE = 0,
	USB_UVC_SENDING_V_STATE,
	USB_UVC_SEND_V_DONE_STATE,
    USB_UVC_SENDING_A_STATE,
	USB_UVC_SEND_A_DONE_STATE,
	USB_UVC_NOT_READY_STATE
};
...
#define USBDEVFS_HOST_ADDRED        	_IOW(USB_IOCTL_ID, 12, uint32_t)
#define USBDEVFS_GET_CONNECT_TYPE		_IOW(USB_IOCTL_ID, 13, uint32_t)
#define USBDEVFS_GET_UVC_STATE			_IOW(USB_IOCTL_ID, 14, uint32_t)
#define USBDEVFS_SET_UVC_PARAM			_IOW(USB_IOCTL_ID, 15, uint32_t)
#define USBDEVFS_SET_UVC_AUDIO_PARAM	_IOW(USB_IOCTL_ID, 16, uint32_t)
...
typedef struct gp_uvc_control_s
{
	int (*get_uvc_state)(void);
	int (*send_video_frame)(void* buf, unsigned long len);
	int (*send_audio_data)(void* buf, unsigned long len);
} gp_uvc_control_t;

typedef struct gp_uvc_parameter_s
{
	void* buf;
	int	  len;
} gp_uvc_parameter_t;
