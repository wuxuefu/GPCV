...
typedef struct gp_disp_info_s
{
	...
	int32_t outputIndex;
	gp_size_t panelSize;
	gp_disp_output_t outputDev[SP_DISP_OUTPUT_MAX];
...
static int32_t
disp_switch_panel(
	uint32_t newOutputType
)
{
	uint32_t i;
	int32_t ret;

	if (newOutputType >= SP_DISP_OUTPUT_MAX) {
		printk("[%s:%d], unknown output type %d\n", __FUNCTION__, __LINE__, newOutputType);
		return -EIO;
	}

	/* Is new panel available? */
	if (gpDispInfo->outputDev[newOutputType].ops == NULL ||
		gpDispInfo->outputDev[newOutputType].ops->init == NULL) {

		DEBUG("[%s:%d], output device %d un-registered\n", __FUNCTION__, __LINE__, newOutputType);
		return -EIO;
	}
	...
	/* Switch to new panel */
	gpDispInfo->outputIndex = newOutputType;
...
)
{
	...
	for (i=0; i<SP_DISP_OUTPUT_MAX; i++) {
		if (gpDispInfo->outputDev[i].ops) {
			return disp_switch_panel(i);
		}
	}
	printk("[%s:%d], Fail, no output device registered\n", __FUNCTION__, __LINE__);
	return -EIO;
}

static int
disp_get_output(
	gp_disp_output_t *poutputDev
)
{
	DEBUG("[%s:%d] type=%d\n", __FUNCTION__, __LINE__, poutputDev->type);

	if (poutputDev->type >= SP_DISP_OUTPUT_MAX) {
		printk("[%s:%d], unknown output type\n", __FUNCTION__, __LINE__);
		return -EIO;
	}

	poutputDev->mode = 0;
	/* if type < 0, then get current output type */
	if (poutputDev->type < 0) {
		if (gpDispInfo->outputIndex < 0)
			return -EIO;
		poutputDev->type = gpDispInfo->outputIndex;
		strncpy(poutputDev->name, gpDispInfo->outputDev[gpDispInfo->outputIndex].name, 32);
		return 0;
	}

	if (gpDispInfo->outputDev[poutputDev->type].ops) {
		strncpy(poutputDev->name, gpDispInfo->outputDev[poutputDev->type].name, 32);
		return 0;
	}
...
)
{
	...
	if (gpDispInfo->outputDev[outputIndex].ops->resume_post)
		(gpDispInfo->outputDev[outputIndex].ops->resume_post)();
...
)
{
	...
	if (panelType >= SP_DISP_OUTPUT_MAX) {
		printk("[%s:%d], Fail, unknown panel type\n", __FUNCTION__, __LINE__);
		return -1;
	}

	/* Check if output type is occupied */
	if (gpDispInfo->outputDev[panelType].ops) {
		printk("[%s:%d], Fail, this output type had been registered\n", __FUNCTION__, __LINE__);
		return -1;
	}

	/* Register panel */
	gpDispInfo->outputDev[panelType].type = panelType;

	memset(gpDispInfo->outputDev[panelType].name, 0, sizeof(gpDispInfo->outputDev[panelType].name));
	strncpy(gpDispInfo->outputDev[panelType].name, name, strlen(name));

	gpDispInfo->outputDev[panelType].ops = panelOps;
...
)
{
	...
	gpDispInfo->outputDev[panelType].ops = NULL;
		...
		case DISPIO_SET_OUTPUT:
			{
				...
				if (copy_from_user((void*) &outputDev, (const void __user *) arg, sizeof(gp_disp_output_t))) {
					err = -EIO;
					break;
				}
				err = disp_switch_panel(outputDev.type);
			}
			break;
		...
		case DISPIO_GET_OUTPUT:
			{
				...
				err = disp_get_output(&outputDev);
				if (copy_to_user ((void __user *) arg, (const void *) &outputDev, sizeof(gp_disp_output_t))) {
					err = -EIO;
				}
			}
			break;
