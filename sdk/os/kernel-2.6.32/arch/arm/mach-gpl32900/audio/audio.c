		...
		while (pplayBuf->working_addr == 0) {
			...
			if (pplayBuf->working_addr == 0) {
				...
				if (++waiting_counter>100) {
					...
					HAL_WRITE_UINT32(APBDMAA_BASE + DMAX_CRX,
							(DMA_M2P | DMA_REQ | DMA_FIX |
							DMA_DOUBLE_BUF | DMA_32BIT_BURST |
							DMA_IRQON | DMA_OFF));
					HAL_WRITE_UINT32(APBDMAA_BASE + DMAX_CRX,
			     ...
			     struct file *filp)
{
	...
	I2SRx_init_mic_agc();
#else /*  */
	//audio_MIC_volset(0x02); /* set by user space */
#endif /*  */
...
static int dsp_suspend(struct platform_device *pdev, pm_message_t state)
{
	if( dsp_devices->w_count>0 ) {
		//Save i2c setting
		dsp_devices->hw.hw_i2s_ctl = halI2sTxCtlGet();
		//clear i2s fifo
		halI2sTxFIFOClear();
		halI2sTxIntDisable();
		halI2sTxDisable();
		...
		HAL_WRITE_UINT32(dsp_devices->dma.iobase + DMAX_CRX, DMA_OFF);
...
static int dsp_resume(struct platform_device *pdev)
{
	...
	audio_Power_ctrl(1);
	dsp_clk_en = 1;
	audio_FREQ_set(dsp_devices->hw.hw_freq, 1);

	audio_WAVE_ctrl(1);
		
	if( dsp_devices->w_count>=0 ) {
		halI2sTxCtlSet(dsp_devices->hw.hw_i2s_ctl);		
		i=1;
		while(1) {
			udelay(1000);
			if( i<dsp_devices->hw.hw_hplvol && i<dsp_devices->hw.hw_hprvol)
				audio_WAVE_volset( i , i );//dsp_devices->hw.hw_hplvol, dsp_devices->hw.hw_hprvol );
			else if( i<dsp_devices->hw.hw_hplvol && i>=dsp_devices->hw.hw_hprvol )
				audio_WAVE_volset( i, dsp_devices->hw.hw_hprvol );
			else if(  i>=dsp_devices->hw.hw_hplvol && i<dsp_devices->hw.hw_hprvol )
				audio_WAVE_volset( dsp_devices->hw.hw_hplvol, i );
			else {
				audio_WAVE_volset( dsp_devices->hw.hw_hplvol, dsp_devices->hw.hw_hprvol );
				break;
			}
			i=i*2;	
		}
...
static int dsp_module_init(void)
{
	...
	HAL_WRITE_UINT32((unsigned int)(APBDMAA_BASE+DMAX_RST), 1);
	
	audio_Power_ctrl(1);
	dsp_devices->hw.hw_freq = 48000;
	dsp_clk_en=1;
	
	audio_WAVE_ctrl(1);
	udelay(1000);
	audio_WAVE_volset(0x1, 0x1);	/*de<input type="button" >fault volume to about 50% */
	udelay(1000);
	audio_WAVE_volset(0x2, 0x2);
	udelay(1000);
	audio_WAVE_volset(0x4, 0x4);
	udelay(1000);
	audio_WAVE_volset(0x8, 0x8);
	udelay(1000);
