...
static long gp_usb_fops_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	struct gp_board_usb_s *pConfig = gp_board_get_config("usb", gp_board_usb_t);
	int host_phy = gpHalUsbHostConfigGet();
