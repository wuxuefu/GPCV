...
#else
	gpHalScuClkEnable(SCU_A_PERI_PPU_REG | SCU_A_PERI_PPU_STN, SCU_A2, 1);
#endif
	gpHalCsi1Init();
...
)
{
	...
	switch(cmd) {
		...
		case VIDIOC_QBUF:
			...
			csi1_devices->bfaddr[idx] = (int)gp_user_va_to_pa((int16_t *)bf->m.userptr);
		...
		case VIDIOC_DQBUF:
			...
			if(csi1_devices->md_enable && csi1_devices->md_outque[0]) {
				...
				csi1_devices->bf[csi1_devices->out_que[0]].reserved = 0x00;
			}
...
#if USE_PPU_IRQ == 1 	
	free_irq(IRQ_2D_ENGINE, csi1_devices);
#else
...
#else 
	ret = request_irq(IRQ_SENSOR, gp_csi1_irq_handler, IRQF_SHARED, "CSI1_IRQ", csi1_devices);
#endif
	if (ret < 0) {
