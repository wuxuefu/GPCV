...
* @return 	none
*/
void gpHalUsbHostRst ( void )
{
    scuaReg_t *pScuaReg = (scuaReg_t *)LOGI_ADDR_SCU_A_REG;
    
    pScuaReg->scuaPeriRst |= SCU_A_PERI_USB0 ;
    pScuaReg->scuaPeriRst &= ~SCU_A_PERI_USB0 ;
}
...
)
{
	...
	if(en == 0)
	{
		...
		if(usbHostUsage <= 0)
		{
			usbHostUsage = 0;
			gpHalUsbHostRst();
			gpHalScuClkEnable(SCU_A_PERI_USB0, SCU_A, 0);
		}
	}
	else
	{
...
{
	unsigned int reg = SCUA_USBPHY_CFG;
