...
static bool bVAlign; /* do vfactor align */
...
)
{
	...
	case SE_CFMT_YCbCr422:
	case SE_CFMT_4Y4Cb4Y4Cr:
		startAlignMask = (~0x7); /* 8 align */
		widthAlignMask = (~0xF); /* 16 align */
		break;
	case SE_CFMT_YCbCr400:
	case SE_CFMT_YCbCr444:
	case SE_CFMT_RGB888:
	case SE_CFMT_YCbCr:
		startAlignMask = widthAlignMask = (~0x3); /* 4 align */
		break;
	default:
		startAlignMask = widthAlignMask = (~0x1); /* 2 align */
		break;
	}
	...
	ret = scaleSetSrcFmt(fmt);
	if(ret != SP_OK) {
		DIAG_ERROR("[scaleSetSrcImg] set input format error.\n");
		return SP_ERR_FORMAT_UNSUPPORTED;
	}
...
)
{
	...
	if ((sclWidth > SCALE_HW_WIDTH_LIMIT) && (sclWidth != actWidth) && (sclHeight != actHeight)) {
		...
		if(sclCtrl & (2 << 16)) {
			DIAG_ERROR("scale1 not support YUVSep+Drop.\n");
			return SP_ERR_HW_LIMIT;
		}
	}
		...
		if (scaleMethod == SCALE_METHOD_DUP_DROP) {
			...
			if (sclHeight > actHeight) {    /* vertical scaling up */
				...
				if (bVAlign &&
					//(sclWidth <= actWidth) &&
					(sclFactorV & 0xFF)) {
					/* HW issue: low bits must be 0 for avoid data eror, then need reset */
					UINT32 factor = (((sclHeight + 1) << 16) + (actHeight - 1)) / actHeight;
					UINT32 newFactor;
					int i = 8;
					...
					while (i) {
						newFactor = (factor >> i) << i;
						if (newFactor > sclFactorV) break;
						i--;
					}
					sclFactorV = newFactor;
				}
				sclCtrl |= (R_SCLCTRL_VSELECT_DN | R_SCLCTRL_VENABLE_FLG);
			}
		}
...
)
{
	...
	if (fmt == SE_CFMT_UNSUPPORTED || fmt >= SE_CFMT_YCbCr400) {
	...
	ret = scaleSetDstFmt(fmt);
	if(ret != SP_OK) {
		DIAG_ERROR("[scaleSetDstImg] output fmt error.\n");
		return SP_ERR_FORMAT_UNSUPPORTED;
	}
...
#if 1
	ret = scaleSetDstRes(sclWidth, sclHeight, actWidth, actHeight);
	if(ret != SP_OK) {
		return SP_ERR_HW_LIMIT;
	}
#else
...
)
{
	...
	gpHalScaleReset();
	if (dither == NULL) {
		gpHalScaleSetDither(SCALE_DITHER_MODE_NONE, 0, 0, 0);
	}
	else {
		gpHalScaleSetDither(dither->mode, dither->sequence, dither->map_upper, dither->map_lower);
	}
