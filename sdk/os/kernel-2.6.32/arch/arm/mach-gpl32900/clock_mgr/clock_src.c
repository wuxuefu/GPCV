...
static void gp_init_clock(struct clk *clk)
{
...
/**
* @brief 	Change arm serial clock
* @param 	clock_buf[in]: arm clock struct buffer
* @param 	rate[in]: clock rate to change
* @return 	SUCCESS/FAIL.
*/
int gp_clk_set_arm(gp_clock_group_t *clock_group)
{
	int real_rate=0;
	
	//printk("[%s] ref_clock =%d\n",__FUNCTION__, clock_group->ref_clock );
	//printk("[%s] base_clock=%d\n",__FUNCTION__, clock_group->base_clock);
	//printk("[%s] ahb_clock =%d\n",__FUNCTION__, clock_group->ahb_clock );
	//printk("[%s] apb_clock =%d\n",__FUNCTION__, clock_group->apb_clock );

#if 0
	if (clock_group->ref_clock == 0) {
		printk("Error , can not set clk_ref_arm=0\n");
		return -ENOENT;
	}
	gp_clk_set_rate("clk_ref_arm", clock_group->ref_clock, &real_rate, NULL);
	if (real_rate != clock_group->ref_clock)
		printk("[%s] Set to[%d], Real rate is [%d]\n",__FUNCTION__, clock_group->ref_clock, real_rate);
#endif
	
	//for 32900 arm , need fix rate for 4:2:1 of base:ahb:apb
	if (clock_group->base_clock == 0) {
		printk("Error , can not set clk_arm=0\n");
		return -ENOENT;
	}

	gp_clk_set_rate("clk_arm", clock_group->base_clock, &real_rate, NULL);
	if (real_rate != clock_group->base_clock)
		printk("Set clk_arm to[%d], Real rate is [%d]\n", clock_group->base_clock, real_rate);

	return 0;
}
EXPORT_SYMBOL(gp_clk_set_arm);

/**
* @brief 	Change aarm serial clock
* @param 	spll[in]: 0:SPLL, 1: SPLL2
* @return 	SUCCESS/FAIL.
*/
int gp_arm_use_spll(int spll_val)
{
	int value = spll_val&0x1;
	struct clk *clkp;
	...
	//re-init clk_ref_arm clock variable
	clkp = clk_get(NULL, (char *)"clk_ref_arm");
	if (IS_ERR(clkp) || clkp == NULL){
...
EXPORT_SYMBOL(gp_arm_use_spll);

/**
* @brief 	dump all clock
* @param 	none
* @return 	SUCCESS/FAIL.
*/
int gp_dump_clock(void)
{
	struct clk *p;
	struct clk **clkp;
...
EXPORT_SYMBOL(gp_dump_clock);


/**
* @brief 	Change ceva serial clock
* @param 	clock_buf[in]: arm clock struct buffer
* @param 	rate[in]: clock rate to change
* @return 	SUCCESS/FAIL.
*/
int gp_clk_set_ceva(gp_clock_group_t *clock_group)
{
	int real_rate=0;
	int ceva_ratio=0;
	...
	//for 32900 ceva , need fix rate for 4:2:1 or 2:2:1 of base:ahb:apb
	if (clock_group->base_clock == 0) {
		printk("Error , can not set clk_ceva=0\n");
		return -ENOENT;
	}

	ceva_ratio = clock_group->ahb_clock/clock_group->apb_clock ;
	if (!(ceva_ratio == 2)) {
		printk("Error , ceva ahb/apb clock ratio not 4:2:1 or 2:2:1\n");
		return -ENOENT;
	}

	ceva_ratio = clock_group->base_clock/clock_group->ahb_clock ;
	if (!((ceva_ratio == 1) ||(ceva_ratio == 2))) {
		printk("Error , ceva base/ahb clock ratio not 4:2:1 or 2:2:1\n");
		return -ENOENT;
	}


	gp_clk_set_rate("clk_ceva", clock_group->base_clock, &real_rate, NULL);
	if (real_rate != clock_group->base_clock)
		printk("Set clk_ceva to[%d], Real rate is [%d]\n", clock_group->base_clock, real_rate);

	gp_clk_set_rate("clk_ceva_ahb", clock_group->base_clock, &real_rate, NULL);
	if (real_rate != clock_group->base_clock)
		printk("Set clk_ceva_ahb to[%d], Real rate is [%d]\n", clock_group->base_clock, real_rate);

	return 0;
}
EXPORT_SYMBOL(gp_clk_set_ceva);
