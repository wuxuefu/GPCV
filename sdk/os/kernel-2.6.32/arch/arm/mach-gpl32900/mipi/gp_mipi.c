...
)
{
	...
	bits = MIPI_CCIR_SOF|MIPI_HD_1BIT_ERR|MIPI_HD_NBIT_ERR|MIPI_DATA_CRC_ERR|MIPI_LANE0_SOT_SYNC_ERR;
	if(p_mipi_dev->cfg.lane_num == MIPI_2_LANE) {
		bits |= MIPI_LANE1_SOT_SYNC_ERR;
	}	 
		
	if(enable) {
		gpHalMipiSetIrq(ENABLE, bits);
	} else {
		gpHalMipiSetIrq(DISABLE, bits);
	}
}
...
)
{
	...
	gp_mipi_set_irq(0, DISABLE);
	flag = p_mipi_dev->CurFrameStatus;
	p_mipi_dev->CurFrameStatus = 1;
#if MIPI_IRQ_EN == 1
	gp_mipi_set_irq(ENABLE);
#endif	
	return flag;
}
...
)
{
	...
	gpHalMipiSetEcc(argp->ecc_order, argp->ecc_check_en, data_mask_cnt, argp->check_hs_seq);
	DEBUG("data_mask_time = %dns\n", data_mask_cnt*delta_t);
}
		...
		if(arg) {
			...
			for(i=0; i<4; i++) {
				...
				if(strcmp(sensor_name, sd->name) == 0) {
					...
					p_mipi_dev->ecc.data_mask_time = sensor->mipi_config->data_mask_time;
...
)
{
	...
	nRet = request_irq(IRQ_CF, 
						gp_mipi_irq_handler, 
						IRQF_DISABLED,
						"MIPI_IRQ", 
						p_mipi_dev);
	if (nRet < 0) {
		DERROR("mipi request irq fail\n");
		RETURN(-ENXIO);
	}
