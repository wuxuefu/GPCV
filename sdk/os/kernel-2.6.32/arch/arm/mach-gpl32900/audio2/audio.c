...
static int dsp_suspend(struct platform_device *pdev, pm_message_t state)
{
	if( dsp_devices->w_count>0 ) {
		...
		audio2_HDPHN_volget((long *)(&dsp_devices->hw.hw_hplvol), (long *)(&dsp_devices->hw.hw_hprvol) );
	}
	audio2_suspend();
	return 0;
}
...
static int dsp_resume(struct platform_device *pdev)
{
	int i;
	audio2_Power_ctrl(1);
	audio2_FREQ_set(dsp_devices->hw.hw_freq, 1);
	...
	if( dsp_devices->w_count>=0 ) {
		...
		while(1)
		{
			...
			if( i<dsp_devices->hw.hw_hplvol && i<dsp_devices->hw.hw_hprvol)
				audio2_WAVE_volset( i , i );//dsp_devices->hw.hw_hplvol, dsp_devices->hw.hw_hprvol );
			else if( i<dsp_devices->hw.hw_hplvol && i>=dsp_devices->hw.hw_hprvol )
				audio2_WAVE_volset( i, dsp_devices->hw.hw_hprvol );
			else if(  i>=dsp_devices->hw.hw_hplvol && i<dsp_devices->hw.hw_hprvol )
				audio2_WAVE_volset( dsp_devices->hw.hw_hplvol, i );
			else
			{
				audio2_WAVE_volset( dsp_devices->hw.hw_hplvol, dsp_devices->hw.hw_hprvol );
				break;
			}
			i=i*2;	
		}
...
static int dsp_module_init(void)
{
	...
	audio2_Power_ctrl(1);
	udelay(1000);
	audio2_WAVE_volset(0x1, 0x1);	/*de<input type="button" >fault volume to about 50% */
	udelay(1000);
	audio2_WAVE_volset(0x2, 0x2);
	udelay(1000);
	audio2_WAVE_volset(0x4, 0x4);
	udelay(1000);
	audio2_WAVE_volset(0x8, 0x8);
	udelay(1000);
	audio2_WAVE_volset(0x10, 0x10);
