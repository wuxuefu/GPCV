...
#define MEDIA_ES_TYPE_TXT		(1<<3)
#define MEDIA_ES_TYPE_NV        (1<<4)
...
typedef struct TxtSTParser
{
 ...
 #define RGB8882565(r,g,b) (((((UINT16)r) & 0x1f) << 11) \
                                            |((((UINT16)g) & 0x3f) << 5) \
                                            |(((UINT16)b) & 0x1f))
typedef  enum FontStyle_s {
	Regular = 1<<0,
	Underline = 1<<1,
	Italic = 1<<2,
	Opaque = 1<<3, //for background
	Semi_Tran = 1<<4,//for background
	Tran		= 1<<5, //for background
	FlashOn		= 1<<6 //for background
} FontStyle;

 /*typedef  enum Color_s {
	White	= RGB8882565(255, 255, 255),
	Green	= RGB8882565(0, 128, 0),
	Blue	= RGB8882565(0, 0, 255),
	Cyan	= RGB8882565(0, 255, 255),
	Red		= RGB8882565(255, 0, 0),
	Yellow	= RGB8882565(255, 255, 0),
	Magenta	= RGB8882565(255, 0, 255),
	Transparent = RGB8882565(0, 0, 0)
} Color;*/
 typedef  enum Color_s {
	White	= 0xffff,
	Green	= 0x07e0,
...
typedef enum SccCommand_e {
	RCL = 1<<0,	//9420=RCL, Resume Caption Loadin:pop-on
	ENM = 1<<1,	//94ae=Erase Non-displayed Memory:Clear Buffer
	EDM = 1<<2,	//942c=Erase Displayed Memory:Clear Caption
	WOF = 1<<3,	//8080=Wait One Frame
	EOC = 1<<4,	//942f=End of Caption:Display Caption
	RU2 = 1<<5, //9425=roll-up 14 and 15
	RU3 = 1<<6, //9426=roll-up 13 through 15
	RU4 = 1<<7, //9427=roll-up 12 through 15
	CR  = 1<<8, //94ad=for roll up,move cursor to bottom-left position of caption and move all lines up
				// does nothing with pop-on or paint-on module
	RDC = 1<<9, //9429=Resume Direct Captioning:paint on
	TR  = 1<<10,//942a=Test Restart
	RTD = 1<<11,//94ab=Resume Text Display
}SccCommand;


typedef struct SccPackage_s {
	UINT16 	sccCmd;	//scc command:pop-on \roll-up\paint-on\clear caption 
	UINT8 	row;	//caption row.value:1~15
	UINT8	column;	//caption column.value:0~31; PAC command
	UINT16	color;	//caption color
	UINT16 	style;	//caption style:italic,underline
	UINT16 	b_color; //background color
	UINT8 *	pSccData;	//caption data
	UINT16	SccSize;	//caption data size
...
typedef struct mcp_demux_info_s
{
	UINT32 client_id;
	SINT8 *file_name;
	UINT32 cid;
...
// init demux
SINT32 demux_open(mcp_demux_info_t *demux_info);
SINT32 demux_duration(SINT8 *file_name);
SINT32 demux_change_url(SINT8 *file_name, UINT32 ms);
...
SINT32 demux_set_s_framerate(UINT32 value);
...
//for ex 
SINT32 demux_set_es_continue(void);
SINT32 demux_get_es_buffer(SINT32 *value);

//for scc caption
SINT32 SccParseCaption(UINT8 *pSccData, UINT32 *Cmd, sccPackage_t *pPack);
