...
# ###############################################################
ifneq ($(KERNEL_SRC),)
KERNEL_BUILD_IMAGE := $(KERNEL_SRC)/arch/arm/boot/zImage
KERNEL_OUT_DIR := $(OUT_DIR)/kernel
#KERNEL_OUT_IMAGE := $(KERNEL_OUT_DIR)/Image
KERNEL_OUT_IMAGE := $(PRODUCT_DIR)/Image
...
.c.o:
	$(MSG_SPLIT_LINE)
	$(MSG_COMPILING)
	@$(TARGET_CC) $(TARGET_GLOBAL_CFLAGS) $(CFLAGS) -MM -MT $@ -o $*.d $<
	@$(TARGET_CC) -c $(TARGET_GLOBAL_CFLAGS) $(CFLAGS) -o $*.o $<

.S.o:
	$(MSG_SPLIT_LINE)
	$(MSG_COMPILING)
	@$(TARGET_CC) $(TARGET_GLOBAL_CFLAGS) $(CFLAGS) -MM -MT $@ -o $*.d $<
	@$(TARGET_CC) -c $(ASFLAGS) $(TARGET_GLOBAL_CFLAGS) $(CFLAGS) -o $*.o $<
